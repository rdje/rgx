...
- Explicit boundaries still in place:
  ...
  - the current wasm ABI keeps `module:function` -> exported `() -> i32` and adds `rgx` host imports for current position, full input text, numbered captures, and named captures; variables are still deferred
