...
- Status: `in-progress 2026-07-21`; **no longer gated on PGEN** — the fast parser (1.1.106) is adopted and raw parse is now ~4% of `Regex::compile`. The remaining gap is RGX-side (adapter boundary + eager C2 build), owned by `COMPILE-PERF-0078.3`/`.4`. The `<5×` bar is KEPT per ADR 0005.
- Goal: reduce the `Regex::compile` gap to **<5x** of PCRE2, accuracy-preserving — no semantic changes, no PGEN bypass, no behavioural drift. The shipped fix-set must keep 100% of the test suite green and the PCRE2 conformance ratchet unchanged.
- Context: the RGX-side mechanical/refactoring techniques below were the in-scope work; #1 (lazy `Engine::new`) shipped and the others are largely subsumed. The dominant remaining cost is **PGEN's regex parse itself** (the sole parser), not RGX downstream work.
- **Status update 2026-05-19 (fresh re-measure, pin `db6f8c68` / PGEN 1.1.81, Apple Silicon, default allocator, 8-pattern corpus):** PGEN parse is **≈63–86%** of `Regex::compile` wall-clock; p50 **24–188µs/pattern**; **geomean ≈214× vs PCRE2-10.47-no-JIT compile / ≈32× vs PCRE2+JIT** (same-session C baseline). Trend: raw PGEN parse p50 ~2–3.8× faster than the original 1.1.40-era baseline; geomean-vs-no-JIT moved ~360× → ~214×. Still far from the <5× target — the residual gap is **sustained PGEN-side parser work**; there is no RGX-side fix and (per user directive 2026-05-19) **no new PGEN bug report is filed** — `PGEN-RGX-0073` stays an open tracker, metrics captured in the docs. The earlier "~80×" figure was PGEN's own-methodology (mimalloc/internal harness) number, not reproduced by RGX's standard measurement.
- Bench reference: **Book → Performance → "Compile-time performance"** (the canonical, kept-current table) and the persisted bundle `pgen-issues/artifacts/PGEN-RGX-0078/measurements/{pgen_parse_p50.txt,pcre2_compile_p50.txt,ratio_table.md}`. The older `target/benchmark-trends/latest.md` per-pattern "~1083–1971× slower" figures are pre-`Engine::new`-lazy-init and pre-PGEN-1.1.81 — superseded by the 2026-05-19 numbers above. Reproduce: `cargo run --release -p rgx-core --example compile_phase_split --features pgen-parser` + `… --example pgen_compile_perf_dump …`.
...
1. **Lazy artifact construction in `Engine::new`** — ✅ **SHIPPED 2026-04-25**.
   ...
   - **Combined with PGEN bottleneck** (as measured 2026-04-25): total RGX-vs-PCRE2 compile gap on the bench corpus moved from ~1083-1971x slower to ~750-1450x slower — a real but bounded improvement (PGEN parse still 96-100% of the new compile budget). Confirms the PGEN-side work in PGEN-RGX-0073 remains the dominant remaining lever. *(Superseded by the 2026-05-19 re-measure above: on pin `db6f8c68`/PGEN 1.1.81 the geomean is ≈214× vs PCRE2-no-JIT and PGEN parse is ≈63–86% of compile — the conclusion stands, the absolute numbers are fresher.)*
...
- Test gate: the full `scripts/run-local-ci.sh` gate green (verified by receipt + banner), including the PCRE2 conformance ratchet — **currently `pass=12_806` / `fail=4` / `panic=0` / `skip=0`** on pin `db6f8c68` (the `pass=12709 / fail=101` figure was the 2026-04-25 baseline; the ratchet has moved since and the gate enforces whatever the committed baseline is).
...
- **Aho-Corasick for top-level literal alternation** — ✅ **SHIPPED 2026-04-25**.
  - **What landed**: new `rgx-core/src/ac.rs` module (~250 lines) with `extract_literal_alternation` AST analyzer + `build_aho_corasick` constructor. `Program` gained `ac_literal_set: Option<aho_corasick::AhoCorasick>`. Compiler builds the AC automaton at compile time alongside `c2_program`. Engine has new `try_ac_is_match` / `try_ac_find_first` / `try_ac_find_all` methods; `Regex::is_match`, `find_first`, `find_all` in `lib.rs` extended their dispatch chain from 4-tier to 5-tier (AC → DFA → Pike-VM → JIT → interpreter). AC configured with `MatchKind::LeftmostFirst` so alternation semantics match PCRE2's first-branch-wins-on-tie rule; 0-based pattern_id translated to 1-based `matched_branch_number`.
  - **Eligibility**: top-level `Alternation` (walks past `Group { Capturing | NonCapturing }` wrappers; `FlagGroup` disqualifies for v1) where every branch is a `Char` or `Sequence` of `Char`, all single-byte ASCII, ≥2 branches, no empty branches. Correctly excludes the single-arm-alternation case where `matched_branch_number = None` is required.
  - **Measured win** (`cat|dog|bird` over 10K input with sparse matches, release build): **find_first 110 ns/iter, find_all 923 ns/iter, is_match 94 ns/iter**. Compared against PCRE2's alternation 1K from `target/benchmark-trends/latest.md` (find_first 350ns, find_all 4854ns), RGX is now competitive or faster. Closes the long-standing "literal alternation falls out of C2 dispatch" gap. 1108 lib + 30 cli tests pass; PCRE2 conformance ratchet preserved at 12,709 / 101.
...
- **Multi-byte literal prefilter via `memmem`** — ✅ **SHIPPED 2026-04-25**.
  - **What landed**: new `leading_literal_bytes(ast)` extractor in `c2/program.rs` walks the top-level AST (past zero-width nodes, through capturing/non-capturing groups, stopping at the first non-literal byte-consumer) and returns the longest fixed-byte run. Stored on `CompiledC2Program` as `c2_prefix_literal: Option<Vec<u8>>` (≥2 bytes — single bytes stay on the cheaper `c2_prefix_byte` / memchr path). `PrefixScanner` extended with a `literal_finder: Option<memchr::memmem::Finder>` that takes priority over the byte-class filter; SIMD-accelerated Two-Way / BM-Horspool under the hood.
  - **Measured win**: `https?://\S+` find_first on 10K input dropped from **32,483 ns/iter → 3,011 ns/iter** — a **10.8x speedup**. Vs PCRE2's 194 ns the gap closes from 167x slower to ~15x slower. find_all also benefits proportionally. `memmem("http")` hits roughly one candidate per real URL vs `memchr('h')` hitting one per ~13 ASCII bytes.
  - **Out of scope**: doesn't help patterns with no leading literal (email_basic still has no leading run; that needs the full v2 inner-literal prefilter). `literal_simple` (pattern `test`) was already fast via the existing VM's `literal_finder` for pure-literal patterns; this commit closes the gap for non-pure patterns with multi-byte literal prefixes.
