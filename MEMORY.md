...
- active_work_unit: none in progress — `BUILD-FLOW` closed (Path A `make` entrypoint + Path B `--no-default-features` restored + CI guard + `.4` `docs/INTEGRATION.md` downstream handoff guide; full gate + conformance GREEN). All governance + the downstream build fix are done.
- next_action: PNT into the active CODE trees — `PERF-SOTA-GAPS.1` (inner-literal prefilter) → `PCRE2-1047-SYNTAX.1` (A12 capture-return VM) → `CODEBLOCK-EXPANSION.1` (inline-lang ergonomics; likely at-parity). Each = real engine change → full `./scripts/run-local-ci.sh` + PCRE2 conformance ratchet per leaf; one leaf at a time. Blocked/deferred/parked: `COMPILE-PERF-0078`, `RUNTIME-REMEASURE`, `A9-BINDINGS`, `RELEASE-CRATESIO`. Downstream answer for LINKEDSPEC recorded in `docs/tasks/BUILD-FLOW.md` ("Downstream response").
