...
- active_work_unit: none in progress — `BUILD-FLOW` closed (Path A `make` entrypoint + Path B `--no-default-features` restored + CI guard; full gate + conformance GREEN). All governance + the downstream build fix are done.
