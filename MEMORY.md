...
- next_action: PNT — `CONFORMANCE-TESTINPUT15.1` (measure the excluded corpus file; its exclusion comment cites exactly the defect just fixed — ratchet-neutral measurement first), then `DOCTRINE-ADOPT.2` (evidence archetype) / `PERF-SOTA-GAPS.1` (inner-literal prefilter; design notes in the tree file). On a PGEN release fixing 0089: re-run `COMPILE-PERF-0078.1` (bump + REGEX-0098 absorption per `docs/knowledge/pgen-1104-verified-blocked.md` checklist).
