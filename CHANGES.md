...
- Changes:
  - `rgx-core/src/parsing.rs`:
    - Added `PgenAstContent::Json(serde_json::Value)` variant + `collapse_to_json` helper mirroring PGEN's `to_json_value()` byte-equivalence contract.
    - New top-level dispatch: `parse_dump` routes `Json(_)` root content through `convert_typed_regex` walking the unified `serde_json::Value` tree directly. Legacy recursive-envelope walker kept as a defensive fallback for non-Json roots.
    - Full typed-shape coverage: `regex` / `pattern` (2-element `[<first_alt>, <rest>]`) / `concatenation` (flat piece array post 0075/0077 fixes) / `piece` (typed object, branch on `type:"piece"`) / `quantifier` (typed `{type, min, max, greediness}` per slice 6) / `anchor` (typed `{type, kind}` per slice 7+9, 11 stable kinds incl. `posix_word_start` / `posix_word_end`) / `posix_class` (typed `{type, name, negated}` per 0076 fix) / `backreference` (typed `{type, kind, ...}` per slice 10, 4 forms with `\g<>`/`\g'…'` → Recursion / `\g{}`/bare → Backreference per PCRE2 bracket-form semantic).
    - All un-annotated atom kinds dispatched by structural prefix per the regex_parser_book Identification table: char_class, all 4 group kinds, all 6 lookaround kinds, conditional (incl. VERSION compile-time short-circuit + alpha-condition assertion `(?(*pla:...)...)`), code_block, comment_group, callout (typed numeric arg), extended_class, alpha-prefixed `(*…)` family (atomic, scs/scan_substring, sr/script_run, alpha lookaround, directive_verb), question-prefixed `(?…)` family (inline_modifiers, scoped_inline_modifiers, subroutine_call).
    - `WhitespaceLiteral` for unescaped whitespace chars so the compiler can strip them under `(?x:...)` extended mode.
    - Bare `\E` outside `\Q\E` lowered to empty Sequence (PCRE2 no-op).
    - `counted_quantifier_body` Object form `{min, max}` per slices 4-5; `max: null` for unbounded `{n,}`.
  - `subs/pgen` submodule pin: `48a9f06` (1.1.29) → `056f6784` (1.1.40 / contract 1.1.42).
  - PGEN-RGX reports filed: `pgen-issues/PGEN-RGX-0074.yaml` (`\Q...\E` quantifier attachment, closed by PGEN 1.1.31), `pgen-issues/PGEN-RGX-0075.yaml` (multi-piece concatenation drop, closed by PGEN 1.1.34), `pgen-issues/PGEN-RGX-0076.yaml` (posix_class name-loss, closed by PGEN 1.1.37), `pgen-issues/PGEN-RGX-0077.yaml` (`[$1**]` flatten-spread Alternative-peel regression, closed by PGEN 1.1.40), `pgen-issues/PGEN-RGX-0078.yaml` (compile-time gap, PCRE2-relative metric — methodology-correction follow-up to 0073). Each report ships a self-contained protocol-compliant artifact bundle under `pgen-issues/artifacts/PGEN-RGX-007{4,5,6,7,8}/` — repro_input.txt, pgen_contract.json, pgen_parse_outcome.txt, pgen_ast_dump.json (single + family), pgen_inputs/<name>.txt, pgen_parse_outcomes/<name>.json, pgen_ast_dumps/<name>.json, pgen_trace.log, helper source, host_call_site.md. 0078 additionally ships `pgen_iteration_flow/` (a vendorable PGEN-side toolkit: standalone C baselines + self-contained Rust microbench + Cargo.toml/Makefile snippets + one-shot driver script) so PGEN can run the PCRE2-relative comparison without any RGX dependency.
  - RGX-side reproducer helpers: `rgx-core/examples/pgen_qe_dump.rs` (0074), `pgen_concat_dump.rs` (0075), `pgen_posix_class_dump.rs` (0076), `pgen_quoted_run_dump.rs` (0077), `pgen_compile_perf_dump.rs` (0078). Each helper walks a family of patterns and writes embedding-API artifacts to its `pgen-issues/artifacts/PGEN-RGX-007{4,5,6,7,8}/` directory.
  - 13 stale `pgen-issues/PGEN-RGX-002[1-3]/0027/0028/003[3-9]/0053.yaml` files flipped from `status: open` to `status: closed` with `closed-by-directive` resolution notes, completing the 2026-04-24 user directive's bookkeeping. Only `PGEN-RGX-0073` (PGEN compile-perf track) remains open.
  - `README.md`: PGEN pin reference updated `1.1.26` / `5856f71` → `1.1.40` / `056f6784`. Build note added about the slice-5 release-engineering pivot — `subs/pgen` no longer ships pre-generated parser artifacts; downstream consumers must run `make -C subs/pgen/rust SHELL=/bin/bash regex_parser_bootstrap` after a fresh clone or PGEN bump. PGEN-RGX report ledger updated to "76 of 77 closed". Conformance ratchet baseline updated to `PASS_BASELINE = 12_709 / FAIL_BASELINE = 101`.
  - `RUST_CODEBASE_ANALYSIS.md`: refreshed at session start with line counts, ratchet baseline, perf sprint summary, PGEN-RGX-0073 + 0074 mentions. (Updated again here for the typed-shape adapter cycle.)
  - `MEMORY.md`: backfilled gap-period summary for 2026-04-19 → 2026-04-27 sessions (engine fixes #29-#38, reverse-DFA pipeline, CLI rename, full perf sprint) plus comprehensive 2026-05-01 typed-shape adapter session entry.
- Validation:
  - `cargo build -p rgx-core` clean.
  - `cargo test -p rgx-core --lib` → **1118 passed; 0 failed; 1 ignored** (matches pre-refactor baseline).
  - `cargo test -p rgx-cli` → pass (no rgx-cli changes).
  - `cargo fmt --manifest-path Cargo.toml -p rgx-core -p rgx-cli -p rgx-bench -p rgx-wasm` → clean.
  - `cargo clippy --workspace --all-targets` → no new errors (warnings tolerated per policy).
  - `cargo test --release -p rgx-core --test pcre2_conformance -- --ignored` → 12,693 / 101 (vs `PASS_BASELINE = 12_709`, `-16` cases). The delta is tracked as a follow-up triage task; no new PGEN bug suspected (the `\Q...\E` family is recovered post-0077 fix). Buckets: 70 FN / 29 SM / 6 RGX-permissive / 5 FP / 4 other / 3 compile-other. The known walker gap is the conditional callout-prefix assertion `(?(?C99)(?=...)...)` shape; that's a walker shape extension, not a workaround.
  - End-to-end functional check: `cargo run --bin rgx -- 'abc' 'abc'` returns `0..3` (was `0..1` mid-refactor before the 0075 fix landed in PGEN).
- Notes/impact:
  - **First major adapter rewrite since the PGEN-backed parser cutover.** The walker shifted from `rule_name`-based dispatch on a recursive `PgenAstNode` envelope to structural-shape dispatch on a unified `serde_json::Value` tree, mirroring PGEN's documented `to_json_value()` byte-equivalence contract.
  - **Strict no-workaround discipline**. Every PGEN bug encountered during the adapter work was filed upstream and fixed there; RGX kept its defensive contract-mismatch error surfaces (e.g. `unrecognised array atom prefix`, `expected typed piece object`) rather than silently absorbing malformed shapes. The RGX-side typed walker handles only documented shapes per the regex parser book; new typed shapes coming through PGEN's task #40 atom-subtree campaign will be added incrementally.
  - **Conformance regression isolated to walker shape coverage**. The -16 ratchet delta concentrates in semantic-divergence buckets (FN/SM/permissive); the `compile: PGEN AST contract mismatch` bucket dropped from 1 to 0 with the 0077 fix. Triage in a follow-up session.
  - **PGEN release-engineering pivot absorbed**. `make regex_parser_bootstrap` is the idempotent cold-clone build target. Documented in `README.md`'s Build, test, run section.
  - Per PCRE2 conformance residual catalogue (`book/src/internals/pcre2-conformance-residual.md`), the residual buckets are mostly stable; the next high-leverage moves remain: Bucket 5 compile-time rejection sweep (4 cases, low risk), Cluster 2C `\K` inside `{0}` (3 cases), Cluster 1C `(*napla:...)` non-atomic positive lookahead (5+ cases).
...
- Changes (no behaviour changes — measurement and documentation only):
  - `rgx-core/examples/compile_phase_split.rs` — new measurement-only example; times `parsing::parse_pattern`, `Compiler::new().compile_ast(ast)`, `Engine::new(&compiled)`, and `Regex::compile(pattern)` independently across an 8-pattern bench corpus. 1000 samples per pattern after 50 warmup iterations; reports median + min/p50/mean/p99/max distributions and per-phase percentage of total compile time. Not a benchmark crate (criterion lives in `rgx-bench`); good-enough for a phase-split decision.
  - `rgx-core/examples/dump_pgen_artifacts.rs` — new measurement-only example; calls `pgen::embedding_api::parser_embedding_api_contract()` and `parse_grammar_profile_named()` for each pattern and writes the JSON outcomes into `pgen-issues/artifacts/PGEN-RGX-0073/`. Used to capture protocol §5 structured artifacts.
  - `pgen-issues/PGEN-RGX-0073.yaml` — new PGEN bug report under the perf class (`pathological_performance`). Covers all sections of `subs/pgen/docs/contracts/PGEN_PARSER_ISSUE_REPORTING_PROTOCOL.md`: §1 Parser Identity (PGEN commit `48a9f06`, parser release `1.1.29`, integration contract `1.1.31`, family `regex`, profile `regex_default`, surface `pgen::embedding_api`), §2 Host Project Identity (RGX commit `c3104ae`, macOS aarch64 Apple M4 Pro 12-core, Rust 1.95, default+jit+pgen-parser), §3 Exact Reproducer Input (8 unedited patterns), §4 Bug Class (`pathological_performance`), §5 Structured PGEN Artifacts (contract JSON, 8 outcome JSONs, 60.5 KB AST dump for `\b\w+@\w+\.\w+\b`, 7.1 MB `PGEN_TRACE_VERBOSITY=debug` parseability_probe trace), §A Reproduction Procedure (commands), §D Performance Fields (1000-sample methodology, trace disabled, corpus-wide scope, p99/p50 ratios, input byte sizes per pattern, AST input/output ratio observation).
  - `pgen-issues/artifacts/PGEN-RGX-0073/` — full evidence bundle (12 files, ~7 MB total). Also includes the PCRE2-10.46 baseline source (`pcre2_compile_baseline.c`, batched 10000-compile loop) and its measured output for reproducibility.
  - `ROADMAP.md` — added two sections under `## Next (near-term) — continued`:
    1. **SOTA algorithmic gaps not on the original C1/C2 roadmap** — surveys 9 SOTA techniques (inner-literal prefilter, Aho-Corasick for literal alternation, SIMD byte-class lookup, tagged DFA, multi-byte memmem, DFA minimization, materialized DFA, Glushkov NFA, anchored fast-paths) graded by win category against the bench numbers in `target/benchmark-trends/latest.md`. Each item names "win in what sense", expected magnitude, and concrete implementation pointer.
    2. **Performance: close the PCRE2 compile-time gap to <5x** — 5 RGX-side techniques in priority order (lazy artifact construction, skip-when-unused, defer-JIT, allocation cleanup, trivial-pattern shortcut). Each includes a "win in what sense" sentence, estimated reduction, how-to, and risk. The ruled-out techniques (PGEN bypass, AST semantics changes) are listed explicitly. Validation plan and measurement-gating noted.
  - `RUST_CODEBASE_ANALYSIS.md` — added a "Next" bullet pointing at PGEN-RGX-0073 as the precondition for the `<5x` compile target.
- Validation: `cargo fmt`, `cargo test -p rgx-core`, `cargo test -p rgx-cli`, `cargo clippy --workspace --all-targets` all clean. PCRE2 conformance ratchet not measured this commit (no engine code touched); preserved at `12,709 / 101 / 0 / 0` from commit `fc20629`. The two new examples build clean and run successfully (output captured in the artifact bundle).
- Notes/impact: this is a measurement and planning commit, not an implementation. It establishes the PGEN bottleneck as the gating constraint for any compile-time improvement and routes the fix through the proper PGEN bug-report channel per CLAUDE.md's `PGEN is the sole parser` rule. The RGX-side techniques in the new ROADMAP entry remain valid follow-up work and are independently implementable; expected total speedup from RGX-side work alone is ~1.2-1.5x (a meaningful but bounded win). The PGEN-side fix carries the rest of the gap.
...
- Changes:
  - `rgx-core/src/c2/dfa.rs` — new public `LazyDfa::find_match_start_at_reverse_bounded(input, end, min_start)`. Walks backward from `end` like the existing method but stops when `pos == min_start`; the zero-width fallback path only records a tentative accept at `end` if `end >= min_start`. The existing `find_match_start_at_reverse` now delegates with `min_start = 0`, preserving its original contract and all 7 existing test callers.
  - `rgx-core/src/engine.rs` — `try_dfa_find_all` mirrors `try_dfa_find_first`'s shape: gates on `pipeline_dispatch_preferred`, pulls forward-unanchored + reverse-anchored mutexes, and delegates to new `try_pipeline_find_all`. The driver iterates: `forward.find_first_accept_at(input, pos)` → first-accept end, `reverse.find_match_start_at_reverse_bounded(input, first_accept_end, pos)` → leftmost start in `[pos, first_accept_end]`, `dfa.find_match_at(input, start)` → greedy end, `pike_captures_at` → captures + end cross-check. Applies find_all's standard advance rules (non-empty → end, empty adjacent to previous non-empty → skip by 1, empty otherwise → `start + 1`). The pre-existing per-position scan remains as the non-pipeline fallback path, reached either when the gate fails (prefix-rich patterns) or when the pipeline exhausts mid-scan.
- Validation: 1,077 lib tests pass (1,071 previous + 6 new — `pipeline_find_all_*` covering non-greedy non-overlapping, greedy extended spans, single char class, empty-match pattern with advance rules, no-match, and a bounded-reverse-walk regression pin on `\w+` over "aa bb cc"). 30 rgx-cli tests pass. PCRE2 conformance ratchet preserved at **12,709 / 101**. `cargo fmt` + `cargo clippy -p rgx-core --lib --tests` clean.
- Notes/impact: closes the ROADMAP "Tier-2 perf headroom — reverse-DFA pipeline" track in full. Next C2-adjacent items in BACKLOG are architectural: multi-byte literal prefix via `memmem::Finder` (minor), smarter Pike-VM dispatch heuristic (unproven). Non-C2 pivots available: A8 crate publishing (blocked on PGEN strategy decision), binary rename `rgx-cli` → `rgx`, or a broader PCRE2 perf sweep.

### 2026-04-24 - C2: wire `find_first` onto the reverse-DFA pipeline

- Scope: the reverse-DFA pipeline foundation has been in place since 2026-04-12 (both forward-unanchored and reverse-anchored lazy DFAs built on `Engine`) but `find_first` still went through the per-position anchored scan because the forward-unanchored DFA's subset construction is leftmost-LONGEST by nature — `find_match_at` records matched_end at every accept during the walk, so for patterns like `a` on `"xaxa"` it would return end=4 (the last 'a'), not the leftmost end=2. ROADMAP had this blocked on "teach the unanchored NFA to kill its lazy-prefix threads after accept".
- Changes:
  - `rgx-core/src/c2/nfa.rs` — `Nfa` carries two new fields: `lazy_prefix_states: Vec<NfaStateId>` (the 4 states allocated by `build_unanchored_prefix` — the `(?s:.)*?` wrapper) and `body_entry: Option<NfaStateId>` (the state the prefix.accept → body.start epsilon lands on). `build_unanchored` / `build_reverse_unanchored` populate both; anchored NFAs leave them at `Vec::new()` / `None`. Accessors `lazy_prefix_states()` / `body_entry()`. New private helper `build_unanchored_prefix_tagged` returns the fragment plus the NFA state IDs allocated during prefix construction.
  - `rgx-core/src/c2/dfa.rs` — two changes. First, `compute_start_set` / `compute_transition_set` check whether the first-pass epsilon closure contains the accept state and, if yes for an unanchored NFA, re-run the closure excluding lazy-prefix states entirely (via the new `closure_excluding_lazy_prefix` helper which seeds `visited` with the prefix states so traversal can't enter them). For `\d` on "abc123" at pos 3 this reduces the post-accept set from `{S0-S3, body_start, body.accept}` to `{body.accept}` — body_start is orphaned (its only incoming ε is from the pruned S1) so the DFA dies on the next byte instead of re-matching another digit. For `a+`, body_start remains reachable via the body-internal greedy loop (DEC → body_start) so greedy extension continues to work. Second, new public method `LazyDfa::find_first_accept_at(input, start)` — mirrors `find_match_at` but returns on the **first** accept reached, not the **latest**. This is the leftmost-first end-hint the pipeline needs.
  - `rgx-core/src/engine.rs` — `try_dfa_find_first` now runs the full 3-pass pipeline when the new `pipeline_dispatch_preferred` gate says yes: (1) forward-unanchored `find_first_accept_at` → first-accept end, (2) reverse-anchored `find_match_start_at_reverse` → leftmost start, (3) forward-anchored `find_match_at` → greedy end (this is what extends `a+` on `"baaab"` from the first-accept end=2 out to the greedy end=4), then Pike-VM `pike_captures_at` for captures. The gate fires only when the pattern has no prefix hint at all (`c2_prefix_byte.is_none()` and `PrefixFilter::None`); prefix-rich patterns fall through to the existing per-position scan because `memchr` / byte-class skip dominates the pipeline's three full sweeps. Pipeline disagreement between DFA greedy end and Pike-VM end falls back to the per-position scan rather than trusting either in isolation.
- Validation: 1,071 lib tests pass (1,061 existing + 10 new — 5 forward-unanchored DFA pins, 4 `find_first_accept_at` contract pins, 6 public-API pipeline regression pins). 30 rgx-cli tests pass. PCRE2 conformance ratchet preserved at **12,709 pass / 101 fail / 0 panic / 0 skip**. `cargo fmt` + `cargo clippy --workspace --all-targets` clean. Fresh criterion run shows all `find_first` pattern deltas within ±5% vs baseline (noise — PCRE2 shows similar drift on patterns my code doesn't touch); an earlier iteration without the prefix-preference gate regressed `url_simple` by +187% which is what drove the gate.
- Notes/impact: closes the ROADMAP "Tier-2 perf headroom" item for `find_first`. `find_all` remains on the per-position scan — the follow-up needs to iterate match-by-match and bound the reverse walk to `>= prev_match_end`. See `book/src/internals/nfa-dfa-engine.md` for the updated design rationale; the Book's "Deliberately deferred" section for the reverse-DFA pipeline flipped from "remaining work: lazy prefix dies at accept" to "both halves shipped; find_all is the follow-up".
...
- Fix: `rgx-core/src/vm.rs` + `rgx-core/src/compiler.rs`. Introduced parallel `named_groups_all: HashMap<String, Vec<u32>>` on `OptimizingCompiler`, populated by the new `Compiler::collect_named_groups_all` walker (preserves every registration in AST order). New VM constant `CONDITIONAL_KIND_NAMED_GROUP_EXISTS_ANY = 8` with operand `<count: u8> <group_id: u8>...`. `NamedGroupExists` codegen emits the single-id path for one-definition names and the multi-id path for dupnames, with runtime dispatch in `evaluate_conditional_operand` iterating the list and returning true if ANY listed group has completed capture. Single-id map `named_groups` is kept unchanged (used by backrefs, substitute template interpolation, public API).
- Validation: 1,056 lib tests pass (added regression pin `dupnames_conditional_checks_any_definition` with 3 assertions covering alt 1 match, alt 2 match, no-match). 30 rgx-cli tests pass. PCRE2 conformance **12,705 → 12,708 pass** (+3), 105 → 102 fail. Ratchet baselines bumped to `PASS_BASELINE=12_708` / `FAIL_BASELINE=102`. `cargo fmt` + `cargo clippy --workspace --all-targets` clean.
- Notes/impact: closes Cluster 1F (3 of the catalogued 3 FN cases). The Bucket 4 substitute case testinput2:4953 `(?J)(?:(?<A>a)|(?<A>b))/replace=<$A>` is the same root cause in the substitute-template-interpolation path; requires a parallel fix in `Regex::interpolate_replacement` which uses the single-id `named_groups` map (follow-up). **Thirty-eighth engine fix of the session; conformance at ~99.2%**.
...
- Changes: (1) **New Book chapter** `book/src/internals/pcre2-conformance-residual.md` (~870 lines) — surgical per-case catalogue of all 107 residual failures, organised primarily by the 5 harness buckets (`false negative` / `span mismatch` / `false positive` / `other` / `RGX too permissive`) and secondarily by 14 root-cause clusters. Each cluster has a pattern/subject/expected-vs-actual table for every case in it, a root-cause analysis naming the exact RGX module/function that would need to change, and a difficulty estimate. Added a "For a new session: how to read this page" front-matter with bucket-to-entry-point navigation, a reproduce-on-demand recipe (`RGX_CONFORMANCE_DUMP_CAT=""`), and a prioritised session sequence (quick-win ~9 cases → mid-session +11 → architectural sprint +26 → capstone +24, cumulative ~70 closed). Closed with a "when this chapter goes stale, here's how to refresh it" protocol. (2) Linked the new chapter into `book/src/SUMMARY.md` under Part VI between "Project Status & Roadmap" and "Contributing". (3) Updated `docs/BACKLOG.md` section C7 to point at the new chapter as the authoritative starting point. (4) Updated `RUST_CODEBASE_ANALYSIS.md` "High-confidence next actions" section to reference the chapter.
- Validation: no code touched; `cargo fmt` + `cargo clippy` re-verified clean. No behavioural change.
- Notes/impact: a future session opening `SESSION_BOOTSTRAP.md` → `README.md` → `MEMORY.md` → `COMMIT.md` will now discover the residual catalogue via `RUST_CODEBASE_ANALYSIS.md` or `docs/BACKLOG.md` and can walk into the Book chapter, pick a cluster that fits their time budget, and start fixing without re-running the dump-and-classify analysis. The chapter explicitly names the recommended quick-win path (Clusters 2C, 3B, 5 = ~9 cases in 1-2 hours) and the architectural prizes (Clusters 1A+2A = ~24 cases in a multi-day sprint). The per-case tables let a contributor jump directly to any specific case by `testinput` file and line.
...
- Changes: surgical edits to the `Current verified snapshot` section (source-line totals per file, MSRV, PGEN pin, conformance numbers, ratchet baselines, test counts, and the feature-surface catalog widened with every engine fix / new opcode that shipped 2026-04-17 → 2026-04-24 — `AltSplit = 0x47` / `AltScopeBegin = 0x48` / `AltScopeEnd = 0x49` / `Accept = 0xF2` / `BackrefCaseInsensitive = 0x68`, the `ci_override_ranges` scheme for `\P{Lu/Ll/Lt}/i`, subroutine-call flag-scope rewrap, lookbehind must-end-at, subexpr-local alt-boundary stack, and so on). Rewrote the `High-confidence next actions` section to reflect the 108-failure residual with top-of-bucket examples — truly-recursive palindromes, Unicode case-fold multi-codepoint, forward-relative recursion, PCRE2 start-optimization parity, positive `\p{Lu}/i` case-fold expansion, residual substitute-mode divergences. Added the conformance-progression distribution-of-wins (harness fixes vs engine fixes vs parser/adapter fixes vs PGEN submodule absorptions). Reflected the user's 2026-04-24 directive that all 72 PGEN-RGX reports are considered closed (13 YAMLs still carry `status: open` on disk — follow-up batch-flip noted as item 14 in the next-actions list).
- Validation: no code touched; `cargo fmt --check` and `cargo clippy --workspace --all-targets` re-verified clean. No behavioural change.
- Notes/impact: future sessions bootstrapping through SESSION_BOOTSTRAP.md will no longer spend the first pass reconciling the analysis doc against the actual head. The RUST_CODEBASE_ANALYSIS.md refresh does NOT touch Track A (the Book) because this change is internal continuity state, not a user-visible feature — the Book's stale fragments (PGEN 1.1.10 references in `project-status.md` / `pgen-integration.md`, "98%" in `appendices/pcre2-compatibility.md`, `(*THEN) partial` in `docs/CAPABILITY_MATRIX.md` and `docs/PCRE2_COMPATIBILITY_MATRIX.md`, PGEN 1.1.8 reference in `DEVELOPMENT_NOTES.md`) are a separate doc-sweep that would be bundled with its own Track A updates.
