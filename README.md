...
Unicode property classes are now also shipped on the default regex path, including compile-time rejection for invalid property names. Recursion remains intentionally parsed-but-unintegrated, while numeric backreferences and conditionals are now shipped too, including compile-time rejection for missing capture-group and conditional group/name references. The CLI still has no native- or wasm-registration surface (tracked explicitly in the docs/matrices above).
