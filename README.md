...
```
The default RGX build now expects the committed `subs/pgen` submodule carrying the pinned PGEN regex `1.1.9` release.
...
The current integration dependency is now pinned as the `subs/pgen` submodule at PGEN 1.1.9 commit `ac2acb3`.
