...
```
The default RGX build now expects the committed `subs/pgen` submodule carrying the pinned PGEN regex `1.1.75` release (commit `08593d05`, integration contract `1.1.77`). *(Version authority: PGEN's own `PGEN_RELEASED_PARSER_BUG_LEDGER.md`, which records the 0081/0082 fixes this pin is named for as Fixed-in 1.1.75/1.1.77. Note: PGEN's `parser_embedding_api_contract()` under-reports these as `1.1.29`/`1.1.31` — its version constants were not bumped in lockstep with its ledger; this is a known PGEN handoff-surface bug filed as `PGEN-RGX-0086`. The `1.1.75`/`1.1.77` figures here are correct and must not be "corrected" downward to the stale constant values.)* **All grammar-level and typed-shape correctness gaps caught by the conformance harness are fixed in the current pin.** Of the 82 PGEN-RGX reports filed against this codebase (0001–0082), 80 are formally closed. The remaining perf pair: **`PGEN-RGX-0073` is the substantive, still-open compile-time perf tracker** — PGEN's `regex` parse is 65–99% of RGX's `Regex::compile` budget; PGEN has optimised it hard (Optim #1–#15 + mimalloc + cached worker thread + inline fast path, embedding-API geomean vs PCRE2-no-JIT ~360x → ~80x) but it is **not yet in an acceptable range**, and the structural ~16x gap to the ROADMAP's "<5x of PCRE2 compile" target still needs specialised regex-grammar codegen or sustained parser hot-path work. **`PGEN-RGX-0078` was the PCRE2-relative metric re-file of 0073 and the PGEN maintainer considers it fixed/superseded** — the substantive work lives under 0073; 0078's RGX report YAML stays formally `open` only until the next `subs/pgen` bump runs its verification-on-close. Separately, PGEN has worked **0084** and is about to address **0085 / 0086**, shipping all three in one combined release. A second submodule `subs/pcre2` tracks the PCRE2 upstream at the `pcre2-10.47` tag and supplies the full `testinput1..29` corpus for the differential conformance harness in `rgx-core/tests/pcre2_conformance.rs` (run with `cargo test --test pcre2_conformance -- --ignored --nocapture`). That harness now enforces a **ratchet gate** at `PASS_BASELINE = 12_716` / `FAIL_BASELINE = 94` / `PANIC_BASELINE = 0` / `SKIP_BASELINE = 0` — any regression fails CI; any improvement bumps the baselines in the same commit.
