...
Build note: `subs/pgen` no longer ships pre-generated parser artifacts (slice-5 release-engineering pivot, commit `0ed2b2ad`). Downstream consumers must run `make -C subs/pgen/rust SHELL=/bin/bash regex_parser_bootstrap` once after a fresh clone or after every PGEN submodule bump; the target is idempotent and uses the cold-clone bootstrap (`generated/ebnf.rs` seeded via the hand-written Rust EBNF frontend, no Perl required).
