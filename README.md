...
```
The default RGX build now expects the committed `subs/pgen` submodule carrying the pinned PGEN regex `1.1.40` release (commit `056f6784`, integration contract `1.1.42`). **All grammar-level and typed-shape correctness gaps caught by the conformance harness are fixed in the current pin.** Of the 77 PGEN-RGX reports filed against this codebase (0001–0077), 76 are fully closed; only `PGEN-RGX-0073` (PGEN regex-grammar parse-time perf) remains genuinely open. Live measurement at this pin (Apple M-series, release): PGEN parse p50 is **453x-875x slower than PCRE2's FULL compile pipeline** across the 8-pattern bench corpus (93µs-410µs vs 132ns-614ns; geomean ~660x). PGEN's release `1.1.30` declared "perf closure" against PGEN's own PRIMARY <50µs target, but that target is not the closure criterion 0073 was filed for — the ROADMAP's "<5x of PCRE2 compile" goal needs PGEN parse to drop to ~50-200ns on this corpus, which is a 1000-2000x speedup beyond what allocator tuning would deliver. A specialised-codegen path for the regex grammar is the most plausible long-term route. A second submodule `subs/pcre2` tracks the PCRE2 upstream at the `pcre2-10.47` tag and supplies the full `testinput1..29` corpus for the differential conformance harness in `rgx-core/tests/pcre2_conformance.rs` (run with `cargo test --test pcre2_conformance -- --ignored --nocapture`). That harness now enforces a **ratchet gate** at `PASS_BASELINE = 12_709` / `FAIL_BASELINE = 101` / `PANIC_BASELINE = 0` / `SKIP_BASELINE = 0` — any regression fails CI; any improvement bumps the baselines in the same commit.
...
The current integration dependency is now pinned as the `subs/pgen` submodule at PGEN 1.1.40 commit `056f6784` (integration contract 1.1.42, "PGEN-RGX-0077: `[$1**]` flatten-spread peels Alternative"). The integration consumes PGEN's typed-Json AST shapes per the regex parser book at `subs/pgen/docs/regex_parser_book/` — `regex` / `pattern` / `concatenation` / `piece` / `quantifier` / `anchor` / `posix_class` / `backreference` are all annotated and emit typed objects directly; remaining atom alternatives still use the recursive-envelope shape per the per-rule shape tables and are walked structurally.
