...
The Rust API now also ships first replacement-oriented helpers on top of that slice: `replace_first_with_code(...)` and `replace_all_with_code(...)` consume winning-path `Replacement(String)` payloads while preserving non-replacement matches unchanged.
