...
- **PCRE2 10.47 feature-family coverage ~98%** (hand-maintained matrix). **Ground-truth case-level pass rate against PCRE2's own `testinput1..29` corpus: 78.5%** (**11,218 cases parsed, 8,811 pass, 2,407 fail, 0 panic, 0 skip** — measured 2026-04-14 via `rgx-core/tests/pcre2_conformance.rs` against PGEN 1.1.21). Every PCRE2 test case — including those gated on features like `ucp`, `J`/`dupnames`, `U`/`ungreedy`, `literal`, `match_line`, `match_word`, non-default newline conventions, and non-UTF-8 subjects — is executed against RGX. No silent skips, no hidden asterisks. The feature-family percentage counts PCRE2 features as atomic check marks and is naturally optimistic; the case-level number is the honest differential against the authoritative oracle. Raise the case-level number as the bug-triage progresses.
