...
```
The default RGX build now expects the committed `subs/pgen` submodule carrying the pinned PGEN regex **`1.1.77`** release (commit `65b845f0`, integration contract **`1.1.79`**, adopted 2026-05-18 — the combined 0084/0085/0086 release). PGEN's `parser_embedding_api_contract()` now reports these correctly (`REGEX_PARSER_RELEASE_VERSION="1.1.77"` / contract `"1.1.79"`); the former stale-version caveat is retired — **PGEN-RGX-0086** fixed it (consts synced to ledger-latest + a PGEN ledger-drift gate). **All grammar-level and typed-shape correctness gaps caught by the conformance harness are fixed in the current pin.** Of the 85 PGEN-RGX reports filed against this codebase (0001–0087), 83 are closed. **PGEN-RGX-0084** (`\NN` forward-reference octal-vs-backref) and **0085** (parser stack-overflow on deep nesting → clean error) and **0086** (handoff version constants) all closed in this combined release. Open: **`PGEN-RGX-0073`** — the substantive compile-time perf tracker (PGEN's `regex` parse is 65–99% of RGX's `Regex::compile`; optimised hard — embedding-API geomean vs PCRE2-no-JIT ~360x → ~80x — but not yet in an acceptable range; the structural gap to the ROADMAP's "<5x of PCRE2 compile" needs specialised regex-grammar codegen / parser hot-path work; **`PGEN-RGX-0078`** was its PCRE2-relative metric re-file, maintainer-considered superseded); and **`PGEN-RGX-0087`** — the `[89]`-leading multi-digit-escape sub-family (`\8N`/`\9N`) the 0084 fix did not extend to (same parent family as 0084; PCRE2 rejects, post-0084 PGEN re-splits → RGX too permissive at `testinput2:4671`/`:4674`). A second submodule `subs/pcre2` tracks the PCRE2 upstream at the `pcre2-10.47` tag and supplies the full `testinput1..29` corpus for the differential conformance harness in `rgx-core/tests/pcre2_conformance.rs` (run with `cargo test --test pcre2_conformance -- --ignored --nocapture`). That harness enforces a **ratchet gate** at `PASS_BASELINE = 12_805` / `FAIL_BASELINE = 5` / `PANIC_BASELINE = 0` / `SKIP_BASELINE = 0` — **deliberately rebaselined from 12,806/4 on adopting the combined pin**: net −1 = +1 (0084's `\10` case closed) −2 (the tracked, PGEN-side **0087** regression); restores toward 12,807/3 when PGEN ships the 0087 fix. Any further regression fails CI; any improvement bumps the baselines in the same commit.
