...
```
The default RGX build now expects the committed `subs/pgen` submodule carrying the pinned PGEN regex `1.1.10` release (adds PCRE2 VERSION conditionals, closes PGEN-RGX-0016). A second submodule `subs/pcre2` tracks the PCRE2 upstream at the `pcre2-10.47` tag and supplies `testdata/testinput1` + `testoutput1` for the differential conformance harness in `rgx-core/tests/pcre2_conformance.rs` (run with `cargo test --test pcre2_conformance -- --ignored --nocapture`).
...
The current integration dependency is now pinned as the `subs/pgen` submodule at PGEN 1.1.10 commit `8783757`.
