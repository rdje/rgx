...
```
The default RGX build now expects the committed `subs/pgen` submodule carrying the pinned PGEN regex **`1.1.80`** release (commit `5fd20609`, integration contract **`1.1.82`**, adopted 2026-05-19). PGEN's `parser_embedding_api_contract()` reports these correctly (`REGEX_PARSER_RELEASE_VERSION="1.1.80"` / contract `"1.1.82"`; PGEN-RGX-0086's ledger-drift gate keeps them in lockstep). **All grammar-level and typed-shape correctness gaps caught by the conformance harness are fixed in the current pin.** Of the 88 PGEN-RGX reports filed against this codebase (0001–0088), 87 are closed. The `\NN…` octal-vs-backref cluster is fully resolved: **PGEN-RGX-0084** (`\NN` forward-reference), **0085** (parser stack-overflow → 250-deep clean-error ceiling), **0086** (handoff version constants), and **0087** (`[89]`-leading `\8N`/`\9N`; took 3 PGEN releases — rel-1.1.78 over-rejected `\8`/`\9` inside `[...]` and was reported, rel-1.1.80 FIX2.1+.2+.3 is correct) — all closed and verified on the adopted pin. Open: **`PGEN-RGX-0073`** — the substantive compile-time perf tracker (PGEN's `regex` parse is 65–99% of RGX's `Regex::compile`; optimised hard — embedding-API geomean vs PCRE2-no-JIT ~360x → ~80x — but not yet in an acceptable range; the structural gap to the ROADMAP's "<5x of PCRE2 compile" needs specialised regex-grammar codegen / parser hot-path work; **`PGEN-RGX-0078`** was its PCRE2-relative metric re-file, maintainer-considered superseded); and **`PGEN-RGX-0088`** — introduced by 0087's FIX2.3, a *distinct* defect (not a 0087 reopen): the bare-octal `>\377` overflow reject is grammar-only & unconditional, so it over-rejects octal escapes valid under UTF mode (`/\777/I,utf` = U+01FF — PCRE2 accepts; PGEN parses mode-agnostically). A second submodule `subs/pcre2` tracks the PCRE2 upstream at the `pcre2-10.47` tag and supplies the full `testinput1..29` corpus for the differential conformance harness in `rgx-core/tests/pcre2_conformance.rs` (run with `cargo test --test pcre2_conformance -- --ignored --nocapture`). That harness enforces a **ratchet gate** at `PASS_BASELINE = 12_806` / `FAIL_BASELINE = 4` / `PANIC_BASELINE = 0` / `SKIP_BASELINE = 0` — **rebaselined from the interim 12,805/5 on adopting pin `5fd20609` (an improvement — the harness's "NEW BASELINE ELIGIBLE, lock it in" path)**: +2 (0087's `testinput2:4671`/`:4674` closed) −1 (0088's `testinput10:218`); restores toward 12,807/3 when PGEN ships the 0088 (UTF-mode octal) fix. Any further regression fails CI; any improvement bumps the baselines in the same commit.
