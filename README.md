...
Some advanced constructs still remain intentionally parsed-but-unintegrated, including recursion and Unicode property classes. Numeric backreferences and conditionals are now shipped on the default regex path, including compile-time rejection for references to missing capture groups and missing conditional group/name references. The CLI still has no native- or wasm-registration surface (tracked explicitly in the docs/matrices above).
