...
```
The default RGX build now expects the committed `subs/pgen` submodule carrying the pinned PGEN regex `1.1.26` release (commit `5856f71`). **All 66 PGEN-RGX reports filed against this codebase (0001–0066) are closed upstream** as of 2026-04-16 — every grammar-level PCRE2 gap caught by the conformance harness has been fixed and the fix is in the current pin. A second submodule `subs/pcre2` tracks the PCRE2 upstream at the `pcre2-10.47` tag and supplies the full `testinput1..29` corpus for the differential conformance harness in `rgx-core/tests/pcre2_conformance.rs` (run with `cargo test --test pcre2_conformance -- --ignored --nocapture`). That harness now enforces a **ratchet gate** at `PASS_BASELINE = 8_822` / `FAIL_BASELINE = 2_396` / `PANIC_BASELINE = 0` / `SKIP_BASELINE = 0` — any regression fails CI; any improvement bumps the baselines in the same commit.
...
The current integration dependency is now pinned as the `subs/pgen` submodule at PGEN 1.1.21 commit `e617960` (integration contract 1.1.23, "Align regex parser with PCRE2 source audit").
