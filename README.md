...
- **PCRE2 10.47 feature-family coverage ~98%** (hand-maintained matrix). **Ground-truth case-level pass rate against PCRE2's own `testinput1..29` corpus: 78.6%** (**11,218 cases parsed, 8,822 pass, 2,396 fail, 0 panic, 0 skip** — measured 2026-04-16 via `rgx-core/tests/pcre2_conformance.rs` against PGEN 1.1.26; the harness is now ratchet-gated so any regression fails CI). Every PCRE2 test case in the harnessed corpus — including those gated on features like `ucp`, `J`/`dupnames`, `U`/`ungreedy`, `literal`, `match_line`, `match_word`, non-default newline conventions, and non-UTF-8 subjects — is executed against RGX. No silent skips, no hidden asterisks — with one **declared** exception: `testinput15` (PCRE2's match-limiting stress file, 80 cases) is not in the harnessed set. It was excluded because those patterns hung the engine before the 2026-07-21 safety-limits fix; measured since, it runs clean at 68/80 with no hangs, and re-inclusion is tracked as the `CONFORMANCE-TESTINPUT15` task tree. See the Book → Testing Philosophy → "One PCRE2 corpus file is excluded" for the full breakdown. The feature-family percentage counts PCRE2 features as atomic check marks and is naturally optimistic; the case-level number is the honest differential against the authoritative oracle. Raise the case-level number as the bug-triage progresses.
...
- [`DOCTRINE_ENFORCEMENT.md`](DOCTRINE_ENFORCEMENT.md) — **the doctrine-enforcement standard** (RGX's 4th portable architecture, adopted from PGEN 2026-07-21): every mechanizable doctrine is paired with a deterministic check, all run from one registry+driver ([`scripts/check_doctrines.sh`](scripts/check_doctrines.sh)) that the pre-commit hook (E3) and CI (E4) gate on. §10 is RGX's live registry of enforced doctrines.
...
- [`docs/PUBLISH_READINESS.md`](docs/PUBLISH_READINESS.md) — cross-cutting gates that must close before public release. Distinct from BACKLOG (per-feature work); this is the publication checklist itself.
...
**Simplest way — one command builds RGX and all its dependencies:**
```bash
make           # = make build: bootstraps PGEN's parser (once) then `cargo build`
```
`make` hides the PGEN bootstrap: it initializes the `subs/pgen` submodule if
needed, generates PGEN's parser (the one-time step a plain `cargo build` can't do
on a fresh checkout — see the Build note below), then builds. It is **idempotent**
(re-running is a no-op once bootstrapped). Other targets: `make test`,
`make gate` (full local CI), `make help`.

**Using RGX as a git submodule?** Run the bootstrap once against the RGX checkout
before building your own workspace — then your `cargo build` finds PGEN's parser:
```bash
...
```
The default RGX build now expects the committed `subs/pgen` submodule carrying the pinned PGEN regex **`1.1.81`** release (commit `db6f8c68`, integration contract **`1.1.83`**, adopted 2026-05-19). PGEN's `parser_embedding_api_contract()` reports these correctly (`REGEX_PARSER_RELEASE_VERSION="1.1.81"` / contract `"1.1.83"`; PGEN-RGX-0086's ledger-drift gate keeps them in lockstep). **All grammar-level and typed-shape correctness gaps caught by the conformance harness are fixed in the current pin.** Of the 88 PGEN-RGX reports filed against this codebase (0001–0088), 87 are closed. The `\NN…` octal-vs-backref cluster is **fully resolved**: **PGEN-RGX-0084** (`\NN` forward-reference), **0085** (parser stack-overflow → 250-deep clean-error ceiling), **0086** (handoff version constants), **0087** (`[89]`-leading `\8N`/`\9N`; took 3 PGEN releases — rel-1.1.78 over-rejected `\8`/`\9` inside `[...]` and was reported, rel-1.1.80 FIX2.1+.2+.3 is correct), and **0088** (0087-FIX2.3's bare-octal `>\377` reject was grammar-only & unconditional, over-rejecting octal valid under UTF — rel-1.1.81 `db6f8c68` reverts it so PGEN emits octal `>0o377` mode-agnostically as a code point, range/mode being an engine-time decision) — all closed and verified on the adopted pin. The early typed-shape cluster — **PGEN-RGX-0079** (`\o{…}` with a non-octal digit must be a parse error), **0080** (whitespace anywhere inside a `{m,n}` quantifier), **0081** (`\g` bracket form distinguishes subroutine call from back-reference), **0082** (native `(?{native:…})` `code_block` content preservation) — was likewise verified-closed on `db6f8c68` (2026-05-19) via fresh-binary probes cross-checked by the green conformance ratchet. The sole open report is **`PGEN-RGX-0073`** — the substantive compile-time perf tracker. Freshly re-measured on the adopted pin `db6f8c68` (PGEN 1.1.81, 2026-05-19, Apple Silicon, default allocator, 8-pattern corpus): PGEN regex parse is **≈63–86%** of RGX's `Regex::compile` wall-clock, p50 **24–188µs/pattern**, **geomean ≈214× vs PCRE2-10.47-no-JIT compile / ≈32× vs PCRE2+JIT** — improved from the ~360× at the original 1.1.40-era filing (raw PGEN parse ~2–3.8× faster) but still short of the ROADMAP's "<5× of PCRE2 compile". (An earlier "~80×" figure was PGEN's own-methodology number, not reproduced by RGX's standard measurement; the RGX-side ground truth is documented in Book → Performance → "Compile-time performance".) No RGX-side fix — parser-speed work lands in PGEN (sole-parser design); no new PGEN report filed. **`PGEN-RGX-0078`** was its PCRE2-relative metric re-file, maintainer-considered superseded. A second submodule `subs/pcre2` tracks the PCRE2 upstream at the `pcre2-10.47` tag and supplies the full `testinput1..29` corpus for the differential conformance harness in `rgx-core/tests/pcre2_conformance.rs` (run with `cargo test --test pcre2_conformance -- --ignored --nocapture`). That harness enforces a **ratchet gate** at `PASS_BASELINE = 12_806` / `FAIL_BASELINE = 4` / `PANIC_BASELINE = 0` / `SKIP_BASELINE = 0` — **held (not rebaselined) across the rel-1.1.81 (`db6f8c68`) adoption**: closing 0088's `testinput10:218` re-exposes `testinput9:287` (`/(?i:A{1,}\6666666666)/` under `testinput9` = "8-bit library, `#forbid_utf`"), a 1↔1 swap. `testinput9:287` is a **permanent, by-design residual** — RGX is a Unicode/code-point engine with no 8-bit non-UTF mode, so it accepts `\666`=U+01B6 exactly as PCRE2-`,utf` does, while PCRE2's 8-bit library raises error 151; both are spec-correct for their model (not a 0088 reopen, not a PGEN bug, not an RGX defect — see Book → PCRE2 Conformance Residuals → Bucket 6 / Cluster 6A). RGX carries **zero known PGEN-tracked regressions**; the 4 residuals are all by-design / won't-fix. Any further regression fails CI; any improvement bumps the baselines in the same commit.

Build note: `subs/pgen` no longer ships pre-generated parser artifacts (slice-5 release-engineering pivot, commit `0ed2b2ad`), so a fresh checkout has no parser until it is generated. The repo-root `make` / `make bootstrap` does this for you (recommended — it is the single command above). Under the hood it runs `make -C subs/pgen/rust SHELL=/bin/bash regex_parser_bootstrap` once after a fresh clone or after every PGEN submodule bump; the target is idempotent and uses the cold-clone bootstrap (`generated/ebnf.rs` seeded via the hand-written Rust EBNF frontend, no Perl required). True zero-step `cargo build` is not possible from RGX alone — cargo compiles the `pgen` dependency before RGX's own build script, and `subs/pgen` is read-only here; making `cargo build` work with no bootstrap would require PGEN's crate to self-generate via its own `build.rs` (a PGEN-upstream change).
...
The current integration dependency is now pinned as the `subs/pgen` submodule at PGEN 1.1.81 commit `db6f8c68` (integration contract 1.1.83; the `\NN…` octal-vs-backref cluster 0084/0085/0086/0087/0088 fully resolved, adopted 2026-05-19). The integration consumes PGEN's typed-Json AST shapes per the regex parser book at `subs/pgen/docs/regex_parser_book/` — slices 11–42 of the typed-shape campaign progressively annotated `regex` / `pattern` / `concatenation` / `piece` / `quantifier` / `anchor` / `posix_class` / `backreference` (now with proper `\g`-bracket-form distinction per 0081) / typed `escape` / typed `atom` (capturing/named/atomic/branch-reset groups, lookarounds, alpha_lookaround, char_class, callout, code_block with preserved content per 0082, conditional, directive_verb, subroutine_call, inline_modifiers) / typed `class_item` (class_range with escape-typed endpoints, class_quoted_range_atom, class_quoted_literal). The walker dispatch backbone in `rgx-core/src/parsing.rs::convert_typed_atom_object` covers every typed shape; remaining un-typed branches fall through to the structural recursive-envelope walker.
