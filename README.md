...
- **PCRE2 10.47 feature-family coverage ~98%** (hand-maintained matrix). **Ground-truth case-level pass rate against PCRE2's own `testinput1..29` corpus: 79.1%** (11,216 cases parsed, 3,670 pass, 971 fail, 0 panic, 6,575 skip — measured 2026-04-14 via `rgx-core/tests/pcre2_conformance.rs` against PGEN 1.1.21). The feature-family percentage counts PCRE2 features as atomic check marks and is naturally optimistic; the case-level number is the honest differential against the authoritative oracle. Raise the case-level number as the bug-triage progresses.
