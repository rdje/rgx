...
- Source code totals ~48K lines across rgx-core. Top-level modules: vm 8202, lib 7584, parsing 3766, compiler 3547, execution 3062, lexer 2383, engine 1657, parser 1486 (retained for non-`pgen-parser` builds), log 641, ast 548, vars 521, file 513, token 467, regex_set 285, cache 231, bytes 222, unicode_support 197, error 85, events 54, lua 24, rhai 21, pattern 20. **C1 JIT subsystem** `rgx-core/src/c1/` (~7.3K lines): codegen 5938, jit 604, runtime 586, mod 211. **C2 NFA/DFA hybrid subsystem** `rgx-core/src/c2/` (~6.8K lines): nfa 1842, pike 1256, program 1125, byte_class 951, dfa 875, classifier 743, mod 53. rgx-cli ~1825 lines.
- MSRV is **1.88** (`Cargo.toml` `rust-version = "1.88"`), driven by PGEN's `edition2024` requirement plus a transitive `home@0.5.12` bump.
- PGEN **1.1.26** at submodule pin `5856f71aa5795ecbc173d94e07f8a8e0eb8c869a` is the sole parser. **All 66 PGEN-RGX reports filed (0001–0066) are closed** with `verified-fixed-upstream` resolution notes — zero open reports as of 2026-04-16. The most recent closures cover short-form Unicode property escapes, `\Q…\E` inside character classes, variable-length lookbehind with control verbs, Unicode group names, stray `\E` in class bodies, `[:<:]` / `[:>:]` word-boundary aliases, DEFINE-aware lookbehind width analysis, `\C` single-byte escape, callout-condition assertions, `(*UTF8)` / `(*UTF16)` / `(*UTF32)` aliases, and forward-reference validation in scan_substring.
- PCRE2 feature parity: **~98% feature-family coverage** (per `docs/PCRE2_COMPATIBILITY_MATRIX.md`). **Case-level pass rate against PCRE2's full `testinput1..29` corpus: 78.6% (8,822 pass / 2,396 fail / 0 panic / 0 skip across 11,218 cases)** measured by `rgx-core/tests/pcre2_conformance.rs`. A **ratchet gate** is now enforced at the bottom of that test: `PASS_BASELINE = 8_822`, `FAIL_BASELINE = 2_396`, `PANIC_BASELINE = 0`, `SKIP_BASELINE = 0` — any regression fails the test; any improvement bumps the baselines in the same commit. JIT compilation shipped (C1 production cutover 2026-04-11) and is on by default. Full inline flags (`(?imsx)` enable/disable/scoped/combined), `\K`, `\R`, `\N`, `\G`, `\X`, `\C` (A10-adjacent, lowered to any-codepoint), `(?C)` callouts, all backtracking verbs including `(*SKIP:name)` (A11), `(?J)`, relative subroutines/backrefs, `(?P<>)`/`(?P=)`, `\k<>`, comment groups, mode settings, full Unicode `(?i)` case folding (A7), returned-capture subroutines `(?N(grouplist))` (A12, parse + compile to `Call`; full capture-return VM semantics is follow-up), `(?(VERSION op X.Y)...)` conditionals (A13), `(*UTF8)` / `(*UTF16)` / `(*UTF32)` width aliases, `[:<:]` / `[:>:]` word-boundary aliases, PCRE2 short property escapes `\pL` / `\PL`, PCRE2 synthetic classes `Xan` / `Xsp` / `Xps` / `Xwd` / `Xuc` with negation, `napla` / `naplb` non-atomic lookarounds, conservative body-pass-through for `(*scan_substring:...)` / `(*script_run:...)`, DEFINE-aware variable-length lookbehind — all shipped.
...
- Testing: **1,007 lib tests** in `rgx-core` (per current head `5dd85ea`), all passing; includes unit, adversarial, integration, property (256+ cases each), stress/fuzz, doc, CLI, bench, and the `rgx-core/tests/api_smoke_test.rs` (19 tests) guarding the documented public API surface. The `rgx-core/tests/pcre2_conformance.rs` ratchet-gated harness runs separately behind `-- --ignored` with 11,218 PCRE2 corpus cases; its `RATCHET OK` guard is now the merge condition for any change that touches parsing, the adapter, the VM, or the conformance harness itself.
...
- The biggest remaining gaps are now narrower and clearer:
  ...
  - the real PGEN backend is green locally through pinned submodule commit `5856f71aa5795ecbc173d94e07f8a8e0eb8c869a` (PGEN **1.1.26**)
...
- The PGEN dependency is now pinned as `subs/pgen` at commit `5856f71aa5795ecbc173d94e07f8a8e0eb8c869a` (PGEN **1.1.26**). Relative to the previous pin this includes — in chronological bump order — the A12 returned-capture subroutine grammar (`(?N(grouplist))`), the A13 VERSION-conditional grammar, PCRE2 short-form property escapes (`\pL` / `\PL`), `\Q…\E` inside character classes, relaxed `class_range` with zero-width markers around the dash, variable-length lookbehind with control verbs, Unicode group names, the `\C` single-byte escape atom, callout-condition conditionals, `[:<:]` / `[:>:]` POSIX word-boundary aliases, DEFINE-aware lookbehind width analysis, `(*UTF8)` / `(*UTF16)` / `(*UTF32)` aliases, and post-parse capture-list validation for `scan_substring`.
...
The merge condition is now the PCRE2 conformance ratchet (8,822 pass / 2,396 fail / 0 panic / 0 skip). Any change that raises the pass count bumps the baselines in the same commit. Any change that drops the pass count fails CI. The path to 100% is guaranteed one-way from here.

**PGEN-side ledger is clean**: 66 reports filed (PGEN-RGX-0001 through PGEN-RGX-0066), 66 closed, 0 open. Every grammar-level PCRE2 gap caught by the conformance harness has been fixed upstream and the fix is in the current submodule pin.

**Remaining conformance residual (roughly sized from recent CHANGES.md entries):**

1. **Unicode case-folding edge cases** — `ẞ` → `ss`, `ſ` → `s`, and other non-trivial multi-codepoint fold pairs. Real engine work in `rgx-core/src/unicode_support.rs` + the compiler's case-fold lowering. A7 handled the common path; these are the residual long tail.
2. **Forward-relative recursion / backrefs** — `(?+1)` / `(?+N)` / `\g{+N}`. Not yet lowered through the compiler's group-resolution pass.
3. **Non-`\n` newline conventions** — `newline=cr`, `newline=any`, `newline=anycrlf`. RGX's `multi_line` flag currently hardcodes `\n` as the line terminator. Lifting this to a per-match newline policy closes a meaningful bucket.
4. **pcre2test substitute-mode** (`/replace=…` / `/substitute*`) — the conformance harness currently treats these as match tests; they're not, they're replace-and-compare tests. Either teach the harness to execute them via RGX's replace API, or classify them as "not applicable" instead of as match failures.
5. **Stricter compile-error parity** — ~162 patterns where RGX accepts what PCRE2 rejects (e.g. `/x(?U)a++b/`). Identifiable strictness gaps; low priority but well-defined.
6. **Residual adapter shapes** — a handful of `simple_escape` letter literals inside character classes, `[\b]` / `[\c]` adapter branches that still need class_escape extension. Single-commit follow-ups.

**Non-conformance items still on the board:**

7. **A9 — Language bindings (Python, Node, C)** — `large` per language. Deferred 2026-04-09 pending real demand signal. If reactivated, start with C bindings via cbindgen.
8. **A12 capture-return VM semantics follow-up** — A12 parsing + `Call` opcode lowering shipped, but full capture-return semantics (preserving the specified groups across the recursive call) remains follow-up.
9. **A8 — Crate publishing** — metadata ready; `pgen` is a private-submodule path dep not on crates.io. Three options on the table (publish pgen; vendor pgen; make pgen-parser truly optional), pending user decision. Binary rename `rgx-cli` → `rgx` is a coordinated doc-sync follow-up.
10. **Reverse-DFA pipeline (C2 follow-up)** — `is_match` single-pass fast path shipped 2026-04-13. Remaining work: teach the unanchored NFA to kill its lazy prefix threads after accept so subset construction preserves leftmost-first semantics. Blocker for wiring `find_first` / `find_all` onto the reverse-DFA pipeline.
11. **GitHub Pages for The RGX Book** — `blocked` on user enabling GitHub Pro. Workflow `.github/workflows/book.yml` deleted in `3ded2e3`, recoverable from git history.
12. **Performance push to <10x PCRE2 gap** — current literal/email find_first beats PCRE2 (6.4x / 3.4x). Capture at 0.88x. Remaining: opcode fusion, per-opcode bounds-check reduction, capture/backtrack pre-allocation.
