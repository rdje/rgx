...
2. **C1 — JIT compilation** — `major`. The next major engineering push now that C2 has shipped. Closes the speed gap with PCRE2's JIT on patterns the C2 hybrid can't handle. Cranelift is already in the dep tree via wasmtime. Sequenced after C2 so the constant-factor JIT win compounds on top of C2's algorithmic-class improvement.
