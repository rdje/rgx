...
- Perl extended character classes `(?[...])`: `shipped`
  - current shipped subset executes bracket/property terms, bare shorthand terms (`\d`, `\D`, `\w`, `\W`, `\s`, `\S`, `\h`, `\H`, `\v`, `\V`), bare escaped literal/codepoint terms (for example `\n`, `\t`, `\r`, `\x{41}`, `\x41`, and escaped operators like `\-`), unary complement, grouped subexpressions, and same-level left-associative set algebra with `&` binding tighter than `|`, `+`, `-`, and `^`, such as `(?[[a-z]])`, `(?[\d - [3]])`, `(?[\w & [a-z]])`, `(?[\D & [A-F]])`, `(?[\h])`, `(?[\H])`, `(?[\v])`, `(?[\V])`, `(?[\x{41} - [B]])`, `(?[\n | \t])`, `(?[[a-z] - [aeiou]])`, `(?[ ![0-9] ])`, `(?[ [AC] ^ [BC] ])`, `(?[ ([a-z] - [aeiou]) & [b-d] ])`, `(?[ [a-f] | [d-z] & [m-p] ])`, and `(?[ [a-z] - [aeiou] + [0-9] - [5] ])`
