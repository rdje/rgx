...
- Perl extended character classes `(?[...])`: `shipped`
  - current shipped subset executes bracket/property terms, nested ordinary bracket terms using the current ordinary char-class atom subset (for example `[\dA-F]`, `[[:graph:]]`, and `[\p{L}]`), bare ASCII POSIX class terms including negated forms (such as `[:alpha:]`, `[:^alpha:]`, `[:graph:]`, `[:digit:]`, `[:space:]`, `[:word:]`, and the other current ASCII forms), bare shorthand terms (`\d`, `\D`, `\w`, `\W`, `\s`, `\S`, `\h`, `\H`, `\v`, `\V`), bare escaped literal/control/octal/codepoint terms (for example `\a`, `\b`, `\e`, `\f`, `\n`, `\t`, `\r`, `\cA`, `\040`, `\o{101}`, `\x{41}`, `\x41`, and escaped operators like `\-`), unary complement, grouped subexpressions, and same-level left-associative set algebra with `&` binding tighter than `|`, `+`, `-`, and `^`, such as `(?[[a-z]])`, `(?[[\dA-F]])`, `(?[[[:graph:]]])`, `(?[[\p{L}] - [\p{Lu}]])`, `(?[ [:graph:] ])`, `(?[ [:^alpha:] ])`, `(?[ ![:alpha:] ])`, `(?[ [:alpha:] & [a-z\t] ])`, `(?[\d - [3]])`, `(?[\w & [a-z]])`, `(?[\D & [A-F]])`, `(?[\h])`, `(?[\H])`, `(?[\v])`, `(?[\V])`, `(?[\x{41} - [B]])`, `(?[\a | \b | \e | \f])`, `(?[\n | \t])`, `(?[\cA | [B]])`, `(?[\040 | \011 | \o{101}])`, `(?[[a-z] - [aeiou]])`, `(?[ ![0-9] ])`, `(?[ [AC] ^ [BC] ])`, `(?[ ([a-z] - [aeiou]) & [b-d] ])`, `(?[ [a-f] | [d-z] & [m-p] ])`, and `(?[ [a-z] - [aeiou] + [0-9] - [5] ])`
  - wider set-expression forms and additional bare-term families beyond the current bracket/property/nested-ordinary/POSIX/shorthand/escaped-term subset still compile-reject explicitly
