...
- **C8.2.1 Per-verb effects refactor (replaces per-pair patches and scales to N verbs)** —
  ...
  - **Phase 3 (pending, deferred SKIP+THEN closure)**: SKIP keeps eager stack-clear (per design note in `verb_apply_skip`) because deferring would regress SKIP-alone semantics. SKIP+THEN compositions stay unrecovered (testinput1:5447 remains SM). A future Phase 3 introduces a `pending_alt_revival: Option<BacktrackFrame>` side-slot on `ExecContext`: SKIP saves the topmost alt-fallback frame before the eager clear; THEN re-pushes it (and clears `skip_position`/`committed`) before the fail propagation. Generalizes to any verb that eagerly clears the stack: PRUNE could save too, allowing `(*PRUNE)(*THEN)` if PCRE2's spec allows it. **Effort**: `small` (1 day). **Unlocks**: testinput1:5447 + any other SKIP+THEN case if/when it surfaces in the corpus.
