...
| **C1 JIT compilation** | `major` | Constant-factor multiplier (~5-10x) on whichever engine runs. Sequenced after C2 so wins compound. | **Steps 0–4a COMPLETE 2026-04-11.** Step 0: design proposal. Step 1: standalone `c1/` module. Step 2: eligibility check. Steps 3a–3e: literal/char-class/anchor/word-boundary/control-flow/all-six-quantifier codegen via decoder unfolding. **Step 4a (this commit)**: corpus-based differential test harness. JIT-compiles patterns through the C1 path AND runs the same patterns through the existing interpreter, then asserts byte-for-byte match-span equivalence across multiple inputs. The harness wraps the JIT'd function in a scan loop (`jit_find_first_via_scan`) that mimics the interpreter's `find_first` semantics so the two paths can be compared apples-to-apples. 27 new differential tests cover all the JIT-eligible opcode families shipped in steps 3a–3e.4: literals, char classes (greedy and negated), anchors, word boundaries, alternations, all six quantifier flavours, and combinations like `\w+@\w+\.\w+` and `\b\d+\b`. **Result: zero divergences across all 27 tests** — the JIT codegen produces byte-for-byte identical results to the interpreter on every corpus input. **The design doc §1.0 (accuracy first) hard gate is now active** for the existing JIT subset. Default build unchanged at 902 tests; with `--features jit` lib **212 C1 tests pass** (185 from step 3e.4 + 27 new differential). Next: step 4b (capture trail in JIT'd code — extends the JIT'd function signature to take a capture buffer pointer, emits real codegen for `SaveStart`/`SaveEnd` with non-zero group ids). Then step 5 (engine dispatch wiring), steps 6–8. |
