...
|------|--------|-----|--------|
| **C2 NFA/DFA hybrid** | `major` | Algorithmic class change. "Can't hang" guarantee for the common no-backtracking subset. 10x-100x typical speedup on regular patterns. | **Steps 1-5 COMPLETE** — classifier (1), byte-class partitioning (2), forward + reverse NFA + `CompiledC2Program` (3), sparse-set Pike-VM with capture tracking + engine dispatch wiring (4), lazy forward DFA cache + DFA dispatch for `is_match` (5). The differential gate is active across the entire 880-test suite for both Pike-VM and DFA paths. Pike-VM handles `find_first`/`find_all`/`captures` for classifier-positive patterns; DFA handles `is_match` for the stricter DFA-eligible subset (no assertions, no lazy quantifiers, no top-level alternation, no flag groups, no multi-byte char classes). Next: step 6 (lazy reverse DFA — enables proper DFA-driven `find_first`/`find_all` via forward+reverse+bounded-Pike-VM pipeline). 8-step phased plan, multi-week timeline. |
