...
| **C1 JIT compilation** | `major` | Constant-factor multiplier (~5-10x) on whichever engine runs. Sequenced after C2 so wins compound. | **Steps 0–3d.2 COMPLETE 2026-04-11.** Step 0: design proposal. Step 1: standalone `c1/` module with `JitHost` wrapper. Step 2: 50-test eligibility truth table. Step 3a: literal codegen. Step 3b: char classes + simple anchors. Step 3c: word boundaries via runtime helper. Step 3d.1: refactor pos to Cranelift Variable. **Step 3d.2**: control flow + backtracking. Adds the 256-frame stack-allocated bt_stack (`Function::create_sized_stack_slot(StackSlotKind::ExplicitSlot, 4096)`), a `bt_top_var` Variable counter, a `failure_dispatch_block` that pops a frame and dispatches via `cranelift_frontend::Switch` (which handles the SSA-inserted block params for the br_table correctly), and codegen for `Split`/`SplitLazy`/`Jump`/`SetAlternative` opcodes. Two-pass `decode_program` resolves Split/Jump forward byte targets to op indices via `binary_search`. All consuming-op fail edges redirect to `failure_dispatch_block` so backtracking is automatic. The pos-restoration test (`\dxy\|\dab` against `5ab`) directly verifies the saved_pos round-trip. 10 new step 3d.2 tests cover simple alternation, three-branch alternation, char-class alternation, anchored alternation, leftmost-first overlap, partial-first-match backtrack, and nested non-capturing alternation. **Still does NOT touch the engine.** Default build unchanged at 902 tests; with `--features jit` **1037 tests pass** (135 C1 tests). Next: step 3e (optimized quantifier opcodes — `QuestionGreedy`, `StarGreedy`, `PlusGreedy`, etc. — wrap inline subprograms and need recursive codegen). Then step 4 (capture trail + differential gate active), step 5 (engine dispatch wiring), steps 6–8. |
