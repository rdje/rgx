...
|------|--------|-----|--------|
| **C2 NFA/DFA hybrid** | `major` | Algorithmic class change. "Can't hang" guarantee for the common no-backtracking subset. 10x-100x typical speedup on regular patterns. | **Steps 1-4a shipped** — pattern classifier (1), byte-class partitioning (2), forward + reverse Thompson NFA + `CompiledC2Program` (3a+3b), sparse-set Pike-VM with `is_match`/`find_first`/`find_all` (no captures yet) **plus the differential gate against the existing VM is now ACTIVE** (4a). 12 differential corpus suites passing — Pike-VM is byte-for-byte compatible with the existing backtracking VM on every classifier-positive case. All standalone, no engine wiring yet. Step 0 design doc: `docs/C2_NFA_DFA_DESIGN.md`. Next: step 4b (capture tracking + engine dispatch wiring). 8-step phased plan, multi-week timeline. |
