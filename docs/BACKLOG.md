...
- **Open C2 perf levers (future sessions)**:
  - **DFA `\b` / `\B` word-boundary support** — currently `\b` patterns fall off the DFA tier and route to backtracking VM (`is_c2_dfa_eligible` rejects all zero-width assertions). Adding word boundaries to the DFA requires extending `DfaStateKey` with a `prev_byte_was_word: bool` flag and evaluating `ZeroWidthAssertion::WordBoundary` during epsilon closure with the (prev-word, curr-word) context. Effectively doubles state count but unlocks DFA dispatch for `\b\w+@\w+\.\w+\b` and other word-boundary-rich patterns. Effort: `medium` (3-5 days). Expected gain: closes the 3.7× `email_basic` gap vs PCRE2. Reference: Rust `regex-automata::dfa` does exactly this; RE2 too.
    - **Phase 1 ✅ landed 2026-05-12 in `26c4953`**: byte-class word partition (`Regex::WordBoundary` now contributes the word-byte oracle so every byte class is unambiguously word or non-word); `ByteClassMap::class_is_word` helper; `Nfa::has_non_word_boundary_assertions` / `has_word_boundary_assertions` accessors. No dispatch change yet — these are pure infrastructure for the DFA closure rewrite below.
    - **Phase 2 (next)**: extend `DfaStateKey` and `DfaState` with `prev_byte_was_word: bool`. Cache by (NFA set, pw). Add `epsilon_close_word_boundary_aware(set, visited, state, fire_wb)` that traverses `WordBoundary` epsilon edges iff `fire_wb` is true and `NotWordBoundary` iff `fire_wb` is false. The DFA's `compute_transition_set` becomes: (a) read byte class `c`; (b) compute `cw = class_is_word(c)`; (c) re-expand source state's NFA set with `fire_wb = (state.pw != cw)`; (d) find byte transitions; (e) compute closure of targets WITHOUT WordBoundary expansion (deferred to next transition); (f) target state has `pw = cw`. The cache key includes pw so two semantically different states (same NFA set, different prev-byte context) get distinct DFA ids. `LazyDfa::new` gating switches from `nfa.has_assertions()` to `nfa.has_non_word_boundary_assertions()` so `\b` / `\B`-only NFAs are accepted. End-of-input acceptance needs special care: when reaching end-of-input, expand closure with `cw = false` (end-of-input acts as a non-word byte for `\b` evaluation). **Critical finding from a 2026-05-12 prototype attempt**: the `DfaState::is_accept` flag is **context-dependent** for patterns with `\b` — the accept state may only be reachable through a satisfied `WordBoundary` epsilon edge, which is NOT in the stored (deferred) closure. The state's `is_accept` boolean on its own returns `false` for the state preceding a trailing `\b\w+@\w+\.\w+\b` accept, so the simulator misses real matches. Phase 2 needs either (a) a context-aware `is_accept(state, pw, cw)` method that re-expands and checks accept presence on demand, or (b) a third `is_accept_via_wb: bool` flag for "may accept depending on context" plus runtime evaluation at acceptance points (transition, end-of-input). Option (a) is simpler; option (b) is faster on the hot path. The prototype attempt landed at the cache-key + closure rewrite but tripped on this acceptance-check issue with `email_pattern_matching` returning 0 matches instead of 2 — revert was clean since the Phase 1 prep (byte_class + Nfa helpers) is independent and stays in the tree.
    - **Phase 3 (after Phase 2)**: flip `is_c2_dfa_eligible` to allow word-boundary patterns and update the engine dispatch comments. Add a `c2_pike_differential` extension to compare DFA vs Pike-VM on `\b`-bearing patterns.
