...
| **C1 JIT compilation** | `major` | Constant-factor multiplier (~5-10x) on whichever engine runs. Sequenced after C2 so wins compound. | **Steps 0–3e COMPLETE 2026-04-11.** Step 0: design proposal. Step 1: standalone `c1/` module. Step 2: eligibility check. Step 3a: literal codegen. Step 3b: char classes + anchors. Step 3c: word boundaries via runtime helper. Step 3d.1: refactor pos to Variable. Step 3d.2: control flow + backtracking. Step 3e.1: PlusGreedy. Step 3e.2: StarGreedy. Step 3e.3: QuestionGreedy. **Step 3e.4**: lazy variants — `QuestionLazy` (`??`), `StarLazy` (`*?`), `PlusLazy` (`+?`). Reuses the same lowerings as the greedy variants but substitutes `SplitLazy` for `Split` (which swaps the branch ordering: try exit first, fall through to inner on backtrack). Refactor: extracted `emit_question_quantifier` / `emit_star_quantifier` / `emit_plus_quantifier` helpers parameterized by a `lazy: bool` flag — the six decoder arms collapse to one helper invocation each. **All six optimized quantifier opcodes are now JIT-compilable.** Patterns like `a??`, `a*?`, `a+?`, `a*?b`, `a+?b`, `\w+?\z`, plus the canonical lazy-vs-greedy contrast (`a*` returns 3 vs `a*?` returns 0 against `aaa`) all work. 12 new step 3e.4 tests; no bugs caught on the first run. **Step 3e is COMPLETE.** With this commit the JIT covers a meaningful subset of real-world patterns: literals, char classes, anchors, word boundaries, alternations, and all six quantifier flavours. The "simple linear inner" restriction still applies; lifting that is a future widening. **Still does NOT touch the engine.** Default build unchanged at 902 tests; with `--features jit` lib **185 C1 tests pass** (173 from step 3e.3 + 12 new step 3e.4). Next: step 4 (capture trail in JIT'd code + differential gate active across the existing test suite). Then step 5 (engine dispatch wiring), steps 6–8. |
