...
| **C1 JIT compilation** | `major` | Constant-factor multiplier (~5-10x) on whichever engine runs. Sequenced after C2 so wins compound. | **Steps 0–5 COMPLETE 2026-04-11.** Step 0: design proposal. Step 1: standalone `c1/` module. Step 2: eligibility check. Steps 3a–3e: literal/char-class/anchor/word-boundary/control-flow/all-six-quantifier codegen via decoder unfolding. Step 4a: corpus-based differential test harness (27 differential tests, zero divergences). **Step 5 (this commit)**: engine dispatch wiring. The JIT is now wired into `Engine::find_first` / `find_all` / `is_match` so the existing test suite exercises it transparently for JIT-eligible patterns. New `JitProgram` struct in `c1/jit.rs` encapsulates `JitHost + FuncId` (with `unsafe impl Send` documented for the read-only-after-finalize invariant). New `Option<Mutex<JitProgram>>` field on `Engine`, populated at compile time via `build_jit_program_if_eligible` (which excludes top-level alternation patterns to preserve `matched_branch_number`, mirroring the C2 dispatch). New `Engine::should_use_jit` runtime gate (no event observer, no runtime safety limits — same shape as `should_dispatch_to_c2`). New `Engine::try_jit_is_match` / `try_jit_find_first` / `try_jit_find_all` methods, each using `PrefixScanner::new(&self.vm, None)` for skip acceleration. The 4-tier dispatch chain in `Regex::find_first` / `find_all` / `is_match` is now: **DFA → Pike-VM → JIT → interpreter**. **Note**: JIT goes AFTER Pike-VM (deviation from design doc §8) because Pike-VM is the safety net for nested-quantifier patterns where the JIT could blow up exponentially. With `--features jit`: 907 lib tests pass (212 C1 + 695 baseline); default build 695 lib tests pass (c1 module is feature-gated). Next: step 4b (capture trail in JIT'd code), then steps 6–8. |
