...
- **Remaining (the campaign)**: ~250 `ignore` blocks across ~22 chapters. Convert incrementally, **highest-traffic first** (✅ getting-started/* + intro + why-rgx → core-api/* → host-integration/* → real-world/* → advanced/* → appendices/* → internals/*). Each increment: convert a chapter's blocks (hidden `# ` setup; `no_run` for IO/feature-gated; fix any real API drift the gate exposes — that is the value; `text` for non-Rust), add its `#[doc=include_str!]` line, bump `book/.examples-verified-chapters`, in one commit. Gate-affecting where it touches `rgx-core/src` / scripts.
