...
| **C1 JIT compilation** | `major` | Constant-factor multiplier (~5-10x) on whichever engine runs. Sequenced after C2 so wins compound. | **Steps 0–3e.2 COMPLETE 2026-04-11.** Step 0: design proposal. Step 1: standalone `c1/` module. Step 2: eligibility check. Step 3a: literal codegen. Step 3b: char classes + anchors. Step 3c: word boundaries via runtime helper. Step 3d.1: refactor pos to Variable. Step 3d.2: control flow + backtracking. Step 3e.1: PlusGreedy via decoder unfolding. **Step 3e.2**: StarGreedy support via the same decoder-unfolding approach. The lowering puts the `Split` BEFORE the inner subprogram (since `*` allows zero matches): `[Split{exit}, inner_jit_ops..., Jump{back to Split}]`. The Jump targets the Split (NOT inner_start) so each iteration pushes a fresh bt_stack frame, accumulating one frame per successful iteration including the zero-iteration case. Both PlusGreedy and StarGreedy share `compute_jit_op_count` (`inner_count + 2`) and the new factored-out `read_inline_subprogram` helper. Patterns like `a*`, `\d*`, `\w*`, `\s*`, `(?:ab)*`, `\A\d*\z`, `a*b+` are now JIT-compilable. 14 new step 3e.2 tests; no bugs caught on the first run. **Still does NOT touch the engine.** Default build unchanged at 902 tests; with `--features jit` lib **161 C1 tests pass** (147 from step 3e.1 + 14 new step 3e.2). Next: step 3e.3 (QuestionGreedy `?` — single conditional execution, no loop). Then the lazy variants (StarLazy/PlusLazy/QuestionLazy with reversed Split/SplitLazy semantics). After all step 3e substeps: step 4 (capture trail + differential gate active), step 5 (engine dispatch wiring), steps 6–8. |
