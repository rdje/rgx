...
| **C1 JIT compilation** | `major` | Constant-factor multiplier (~5-10x) on whichever engine runs. Sequenced after C2 so wins compound. | **Steps 0–3e.3 COMPLETE 2026-04-11.** Step 0: design proposal. Step 1: standalone `c1/` module. Step 2: eligibility check. Step 3a: literal codegen. Step 3b: char classes + anchors. Step 3c: word boundaries via runtime helper. Step 3d.1: refactor pos to Variable. Step 3d.2: control flow + backtracking. Step 3e.1: PlusGreedy. Step 3e.2: StarGreedy. **Step 3e.3**: QuestionGreedy support — the simplest of the three optimized quantifier lowerings: `[Split{exit}, inner_jit_ops...]` with NO Jump back, because `?` is "zero or one" and there's no loop. Total unfolded count is `inner_count + 1` (not `+2` like Plus/Star). Patterns like `a?`, `\d?`, `\w?`, `(?:ab)?`, `\Aa?\z`, `a?b+` are now JIT-compilable. **All three greedy optimized quantifiers (`+`, `*`, `?`) are now supported.** 12 new step 3e.3 tests; no bugs caught on the first run. **Still does NOT touch the engine.** Default build unchanged at 902 tests; with `--features jit` lib **173 C1 tests pass** (161 from step 3e.2 + 12 new step 3e.3). Next: step 3e.4 (lazy variants `*?` `+?` `??` — same lowerings but use `SplitLazy` to swap branch ordering, trying the "exit"/zero-iteration branch first). After all step 3e substeps: step 4 (capture trail + differential gate active), step 5 (engine dispatch wiring), steps 6–8. |
