...
|------|--------|-----|--------|
| **C2 NFA/DFA hybrid** | `major` | Algorithmic class change. "Can't hang" guarantee for the common no-backtracking subset. 10x-100x typical speedup on regular patterns. | **Steps 1-4 COMPLETE** — classifier (1), byte-class partitioning (2), forward + reverse NFA + `CompiledC2Program` (3), sparse-set Pike-VM with capture tracking + **engine dispatch wiring** (4). The public `Regex::compile` API now automatically routes classifier-positive patterns through the Pike-VM via `Engine::should_dispatch_to_c2()`. **The differential gate against the existing VM is active across the entire 856-test suite** — every classifier-positive pattern produces identical results on both engines. Patterns with top-level alternation, flag groups, `\G`, multi-byte char classes, runtime event observers, or runtime safety limits route to the existing VM (SOTA-correct exclusions; lift each as Pike-VM gains the feature). Step 0 design doc: `docs/C2_NFA_DFA_DESIGN.md`. Next: step 5 (lazy forward DFA cache). 8-step phased plan, multi-week timeline. |
