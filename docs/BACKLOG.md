...
| **C1 JIT compilation** | `major` | Constant-factor multiplier (~5-10x) on whichever engine runs. Sequenced after C2 so wins compound. | **Steps 0–3e.1 COMPLETE 2026-04-11.** Step 0: design proposal. Step 1: standalone `c1/` module. Step 2: eligibility check. Step 3a: literal codegen. Step 3b: char classes + anchors. Step 3c: word boundaries via runtime helper. Step 3d.1: refactor pos to Variable. Step 3d.2: control flow + backtracking. **Step 3e.1**: PlusGreedy support via decoder unfolding. The decoder reads the inline subprogram from `PlusGreedy(inner)`, recursively decodes it via `decode_simple_inner_into`, and unfolds the quantifier into `[inner_jit_ops..., Split{exit}, Jump{back to inner_start}]` using the step 3d.2 backtracking infrastructure. Two-pass `collect_op_positions` returns `Vec<(byte_offset, jit_op_idx)>` so forward Split/Jump targets pointing at PlusGreedy opcodes resolve correctly to the first JitOp in the unfolded sequence. Restricted to "simple linear inner" subset (no nested control flow, no nested optimized quantifiers) for step 3e.1; subsequent substeps will widen. Patterns like `a+`, `\d+`, `\w+`, `\s+`, `(?:ab)+`, `\w+@\w+\.\w+`, `\A\d+\z`, `\d+|word` are now JIT-compilable end-to-end with byte-for-byte correct greedy semantics. 13 new step 3e.1 tests cover all char-class quantifiers, multi-char inners, anchored quantifiers, alternation+quantifier combinations, and the critical "backtrack into quantifier" case. **Still does NOT touch the engine.** Default build unchanged at 902 tests; with `--features jit` lib tests grow to **842** (147 C1 tests). Next: step 3e.2 (StarGreedy via decoder unfolding — similar pattern but with the Split BEFORE the first iteration since `*` allows zero matches). Then step 3e.3 (QuestionGreedy), the lazy variants, step 4 (capture trail + differential gate active), step 5 (engine dispatch wiring), steps 6–8. |
