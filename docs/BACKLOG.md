...
|------|--------|-----|--------|
| **C2 NFA/DFA hybrid** | `major` | Algorithmic class change. "Can't hang" guarantee for the common no-backtracking subset. 10x-100x typical speedup on regular patterns. | **Steps 1-4 COMPLETE; Step 5a shipped** — classifier (1), byte-class partitioning (2), forward + reverse NFA + `CompiledC2Program` (3), sparse-set Pike-VM with capture tracking + engine dispatch wiring (4), **lazy forward DFA cache** standalone module (5a). Pike-VM is now the runtime engine for classifier-positive patterns; the differential gate is active across the entire 856-test suite. The DFA module is built but not yet dispatched. Step 5a deliberately omits assertion support and lazy-quantifier semantics (both lifted in 5b). Next: step 5b (engine dispatch wiring for the DFA + cache exhaustion fallback). 8-step phased plan, multi-week timeline. |
