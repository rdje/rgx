...
- **C8.2.1 Per-verb effects refactor (replaces per-pair patches and scales to N verbs)** — implement the `VerbState` struct + `apply` table from C8.1.1; rewrite each verb's `OpCode::*` arm to compose effects via `verb_table[op].apply(&mut ctx.verb_state, ctx.pos)`; collapse the failure-handling sites (8 scanning-loop sites + SIMD path + assertion-subexpr propagation) into one consumer that reads `verb_state` and decides advance / abort / alt-redirect / accept. Tests: one spec test per verb (verb-vs-PCRE2 effect match) plus ~10 composition tests covering 3-, 4-, 5-verb sequences. The model scales by construction — N verbs apply by `fold(state, |s, v| v.apply(s, pos))` with no special-cased pair / triple / N-tuple logic. Per-pair patches (#24, #36, the 2026-05-05 SKIP-overrides-COMMIT, the held `commit_saved_alt`) collapse into rows of the apply table. **Effort**: `medium` (4-7 days). **Unlocks**: residual Cluster 1D (testinput1:5457 and family) by construction; closes the held `commit_saved_alt` proposal as a special case; eliminates the per-pair commit pattern entirely — there are no future verb-pair commits because there are no pairs.
