...
| **C1 JIT compilation** | `major` | Constant-factor multiplier (~5-10x) on whichever engine runs. Sequenced after C2 so wins compound. | **Steps 0–3d.1 COMPLETE 2026-04-11.** Step 0: design proposal. Step 1: standalone `c1/` module with `JitHost` wrapper. Step 2: 50-test eligibility truth table. Step 3a: literal codegen. Step 3b: char classes + simple anchors via per-opcode block-per-block architecture. Step 3c: word boundaries (`\b`/`\B`) via runtime helper indirect call. **Step 3d.1**: pure refactor — switches `pos` from a per-block parameter to a Cranelift `Variable`. No new functionality, no new tests, no behaviour change. Foundation for step 3d.2 which needs to restore pos from the backtrack stack on failure dispatch (Cranelift `br_table` doesn't accept per-target arguments, so anything restored on backtrack must live in a Variable). All 126 existing C1 tests pass under the rewritten architecture. **Still does NOT touch the engine.** Default build unchanged at 902 tests; with `--features jit` 1028 tests pass (126 C1 tests, unchanged). Next: step 3d.2 (control flow `Split`/`Jump`/`SplitLazy` + 256-frame stack-allocated backtrack array + `failure_dispatch` block with `br_table` — quantifiers + alternation become JIT-eligible). Then step 4 (capture trail + differential gate active), step 5 (engine dispatch wiring), steps 6–8. |
