...
| **C1 JIT compilation** | `major` | Constant-factor multiplier (~5-10x) on whichever engine runs. Sequenced after C2 so wins compound. | **Steps 0–1 COMPLETE 2026-04-11.** Step 0: comprehensive design proposal landed at `docs/C1_JIT_COMPILATION_DESIGN.md` (Cranelift code generator, JIT'd backtracking VM, 4-tier dispatch chain, cross-platform matrix, 9-step phased plan, 12 open questions, 12 risks). Step 1: standalone `rgx-core/src/c1/` module with `JitHost` wrapper around `cranelift_jit::JITModule` (~330 lines + 2 smoke tests), runtime helper signature skeleton (~170 lines + 1 link test), opt-in `jit` Cargo feature gating cranelift-codegen 0.101 (pinned to match wasmtime's transitive version). Smoke test exercises the entire JIT pipeline end-to-end (target ISA selection → IR construction → compilation → finalisation → function pointer transmute → native invocation) by building a tiny `extern "C" fn() -> i64` that returns 42. PIC disabled because cranelift-jit 0.101's PLT only supports x86_64. Default build unchanged (902 tests); with `--features jit` 905 tests pass. **Does NOT touch the engine** — module is completely standalone, removable without affecting any other path. Next: step 2 (JIT eligibility check — AST walker + `Program::jit_eligible: bool` field, still standalone). |
