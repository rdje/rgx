...
| **C1 JIT compilation** | `major` | Constant-factor multiplier (~5-10x) on whichever engine runs. Sequenced after C2 so wins compound. | **Steps 0–3c COMPLETE 2026-04-11.** Step 0: design proposal. Step 1: standalone `c1/` module with `JitHost` wrapper. Step 2: 50-test eligibility truth table. Step 3a: literal codegen. Step 3b: char classes + simple anchors via per-opcode block-per-block architecture. **Step 3c**: word boundaries (`\b`/`\B`) via runtime helper indirect call. Real impl of `rgx_runtime_word_boundary_test` (PCRE2 ASCII semantics — exactly one of the bytes at `pos-1` and `pos` is a word char). Symbol registered with Cranelift `JITBuilder` in `JitHost::new`. New `JitHost::import_word_boundary_helper` declares the helper as `Linkage::Import` inside the JIT'd function and returns a `FuncRef` for indirect calls. New `JitOp::WordBoundary { negated }` variant. The runtime helper infrastructure (symbol registration + function import + indirect call codegen) is now in place and reusable for step 6 (CharClass + multi-byte helpers) and step 7 (runtime safety helpers). 23 new tests (11 helper unit tests + 12 codegen tests). **Still does NOT touch the engine.** Default build unchanged at 902 tests; with `--features jit` **1028 tests pass** (126 C1 tests). Next: step 3d (control flow `Split`/`Jump` + backtrack stack — quantifiers + alternation become JIT-eligible). Then step 4 (capture trail + differential gate active), step 5 (engine dispatch wiring), steps 6–8. |
