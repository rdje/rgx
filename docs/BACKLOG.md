...
| **C1 JIT compilation** | `major` | Constant-factor multiplier (~5-10x) on whichever engine runs. Sequenced after C2 so wins compound. | **Steps 0–3b COMPLETE 2026-04-11.** Step 0: design proposal at `docs/C1_JIT_COMPILATION_DESIGN.md`. Step 1: standalone `c1/` module with `JitHost` wrapper. Step 2: `is_jit_eligible(program)` 50-test truth table. Step 3a: literal codegen for `Char(len=1)` programs. **Step 3b**: refactored `compile_program` to use a per-opcode block-per-block IR architecture (each block takes `pos: i64` as a block parameter, flows between blocks via Cranelift block params). New `JitOp` enum + `decode_program` walker. Adds inline codegen for `DigitAscii`/`DigitAsciiNeg`, `WordAscii`/`WordAsciiNeg`, `SpaceAscii`/`SpaceAsciiNeg`, `StartText`, `EndText`. The `(text, text_len, pos) -> isize` signature is unchanged. 34 new step 3b tests cover every char-class variant + anchor + combination. **Still does NOT touch the engine.** Default build unchanged at 902 tests; with `--features jit` **1007 tests pass** (105 C1 tests = 3 step 1 + 50 step 2 + 18 step 3a + 34 step 3b). Next: step 3c (control flow `Split`/`Jump` + backtrack stack — quantifiers + alternation become JIT-eligible). Then step 4 (capture trail + differential gate active), step 5 (engine dispatch wiring), steps 6–8. Word boundaries (`\b`/`\B`) need a runtime helper call and are deferred until the helper-registration machinery lands. |
