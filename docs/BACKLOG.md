...
- **Remaining**: only the #45 branch-ID family follow-up (host-integration data-exchange/match-steering — `no_run`, non-gating). The campaign mechanism is permanent and self-enforcing (book_doctests.rs on the mandatory `cargo test -p rgx-core` gate + `check-book-examples.sh` count-only-grows ratchet + `book.yml` deploy-gate): any new chapter or `rust` block must be verified to land.
