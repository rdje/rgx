...
- **✅ Whole-attempt coverage closed (2026-07-21, `VM-LIMITS-SUBEXEC.1`)**: the limits used to bound only the top-level dispatch loop, so every sub-execution (assertion bodies, lookbehind candidate starts, quantifier probes, subroutine bodies, async resume) ran off-budget — `/(?<=(\d{1,256}))X/` took 20+ minutes on an 8-byte subject despite `set_max_steps(1_000_000)`. All three dispatch loops now charge one shared per-attempt budget and speculative sub-contexts fold their spend back into the parent (37 ms for that case). This also discharges the "audit every RGX hot path checks `max_steps`" follow-up (see the testinput15 note under the deep-nesting entry): the audit enumerated every dispatch loop and every context-clone site, and all are covered. Regression tests: `rgx-core/tests/adversarial.rs` → `limits_bound_*`.
...
- **Excluded files** (see harness comments for details):
  - `testinput15` — match-limiting stress file with catastrophic-backtracking patterns (`(a+)*zz`). Excluded because some cases didn't honor the harness's `max_steps` cap and hung indefinitely. **Root cause fixed 2026-07-21 (`VM-LIMITS-SUBEXEC.1`)** — sub-executions ran off-budget, so the cap was only ever a top-level-loop cap; the "audit every hot path" follow-up is discharged (see A2). **Re-measured 2026-07-21 (`CONFORMANCE-TESTINPUT15.1`): the file no longer hangs — 80 cases, 68 pass / 12 fail / 0 panic, +65.6s sweep cost.** (The "41 cases" estimate above was low by ~2×.) Of the 12 failures, **10 are a harness gap** — pcre2test's `allusedtext` prints a span including lookaround-consulted text and the harness doesn't adjust the expected side — and 2 are real RGX gaps (pattern-level `(*LIMIT_*)` verbs, infinite-recursion detection). Re-inclusion is a ratchet-moving decision owned by the **`CONFORMANCE-TESTINPUT15`** tree (`.3` closes the harness gap first, then `.2` adjudicates). The exclusion is now **declared** in `README.md` and the Book's Testing Philosophy rather than silent.
...
> releases: 0084/0085/0086 closed at `65b845f0` (rel 1.1.77); 0087's
> `[89]`-leading family took two attempts (rel-1.1.78 over-rejected
> `\8`/`\9` inside `[...]` — caught + reported; rel-1.1.80 FIX2.1+.2+.3
> correct); 0088 (FIX2.3's unconditional octal `>\377` reject) closed by
> rel-1.1.81's scoped revert. RGX now at `subs/pgen` **`db6f8c68`** (rel
> **1.1.81** / contract **1.1.83**, AST schema `1` unchanged):
...
> - **PGEN-RGX-0088 ✅ CLOSED 2026-05-19** — FIX2.3's octal `>\377` reject was grammar-only & UNCONDITIONAL, over-rejecting octal valid under UTF (`/\777/I,utf` = U+01FF). rel-1.1.81 (`db6f8c68`) **reverts the unconditional reject** → PGEN emits octal `>0o377` mode-agnostically as a code point (parser-correct: range/mode is an engine decision, not the parser's — `feedback_ast_pipeline_parser_agnostic`). Verified on `db6f8c68`: `\777`→U+01FF ACCEPT, `\6666666666`→U+01B6+"6666666", `[\666\777]` ACCEPT, `\377` boundary ACCEPT, `\89` still REJECT (0087 family byte-identical). `testinput10:218` CLOSED. New regression test `octal_escape_over_0o377_is_a_codepoint_not_a_parse_reject`.
> **Conformance ratchet stays `12,806/4`** (the rel-1.1.81 revert is a 1↔1
> swap, NOT a rebaseline): `testinput10:218` CLOSES and `testinput9:287`
> (`/(?i:A{1,}\6666666666)/`, `testinput9` = "run only with the 8-bit
> library, #forbid_utf") RE-EXPOSES. **`testinput9:287` is now a
> permanent, by-design residual** — RGX is a Unicode/code-point engine
> with no 8-bit non-UTF mode, so it accepts `\666`=U+01B6 exactly as
> PCRE2-`,utf` would, while PCRE2's 8-bit non-UTF library raises error
> 151. Both are spec-correct for their model; **not a 0088 reopen, not a
> PGEN bug, not an RGX defect** (full rationale: Book → PCRE2 Conformance
> Residuals → **Bucket 6 / Cluster 6A**; `PGEN-RGX-0088.yaml`
> `not_a_reopen_and_not_a_new_bug`). RGX carries **zero known PGEN-tracked
> regressions**; the 4 residuals are all by-design / won't-fix.
> 0073/0078 compile-time re-extract still deferred
> (independent).
...
- ✅ **PGEN-RGX-0088 — CLOSED 2026-05-19** (octal `>\377` over-reject — *introduced by 0087's FIX2.3*, a distinct defect, NOT a 0087 reopen). FIX2.3's bare-octal overflow reject was grammar-only & UNCONDITIONAL, but an octal escape is valid up to the codepoint max under UTF (`/\777/I,utf` = U+01FF, PCRE2 ACCEPTS). Fixed in PGEN rel **1.1.81** (commit `db6f8c68`): a scoped revert — PGEN now emits octal `>0o377` mode-agnostically as a code point (range/mode is an engine-time decision, not the parser's; `feedback_ast_pipeline_parser_agnostic`). Verified on adopted pin `db6f8c68`: `\777`→U+01FF ACCEPT, `\6666666666`→U+01B6+"6666666", `[\666\777]`/`\377`-boundary ACCEPT; `\89` still REJECT and the whole 0087 family byte-identical. `testinput10:218` CLOSED. YAML `status: closed` (+`not_a_reopen_and_not_a_new_bug` block). New regression test `octal_escape_over_0o377_is_a_codepoint_not_a_parse_reject` (lib.rs). **Residual swap, not a regression:** closing `testinput10:218` re-exposes `testinput9:287` (`/(?i:A{1,}\6666666666)/` under `testinput9` = "8-bit library, `#forbid_utf`"). That residual is now **permanent & by-design** — RGX is Unicode/code-point-only (no 8-bit non-UTF mode), so it accepts `\666`=U+01B6 exactly as PCRE2-`,utf` does, while PCRE2's 8-bit library raises error 151. Both spec-correct for their model; **not a PGEN bug, not an RGX defect**. Documented: Book → PCRE2 Conformance Residuals → **Bucket 6 / Cluster 6A** + Appendices → PCRE2 Compatibility → "Engine model: rgx is Unicode-only". Ratchet unchanged at 12,806/4 (1↔1 swap, no rebaseline).
- ✅ **PGEN-RGX-0079/0080/0081/0082 — CLOSED 2026-05-19** (early typed-shape cluster, verified-on-close on the adopted pin `db6f8c68`). **0079** `\o{1239}`/`\o{8}`/`\o{12abc}` (non-octal-digit braced octal) now PARSE-ERROR (PCRE2 error-164 faithful; valid `\o{377}` still octal U+00FF). **0080** `a{ 1 , 2 }`/`a{1 ,2}`/`a{1, 2}` now parse to the same quantifier shape as `a{1,2}` (whitespace anywhere in `{m,n}` ignored, PCRE2 default mode). **0081** typed AST emits distinct `kind`s (`subroutine_named`/`named_braced`/`numeric_backreference`/`subroutine_numeric`); discriminator `^(a|b)\g{1}$` (backref) vs `^(a|b)\g<1>$` (subroutine) is PCRE2-faithful; RGX consumes the typed `kind` directly — the heuristic the report contemplated was never shipped. **0082** native `(?{native:NAME})` `code_block` content preserved; the report's 11 `full_mode_native_*` lib tests all pass. New regression `pgen_rgx_0079_0080_0081_parser_shape_family_is_pcre2_faithful` (lib.rs) + per-report `verification_db6f8c68/` artifacts; YAMLs `status: closed` (`resolution: resolved`, `no_workaround_adopted: true`). Cross-checked by the green conformance ratchet (12,806/4). **Process note:** an initial pass wrongly read these as "still broken" because a stale `target/debug/rgx` binary (pre-`db6f8c68` parser) was probed; rebuilding fresh showed the fix — the bad closures were reverted and redone correctly (lesson: `feedback_rebuild_after_pgen_bump`; no entrypoint divergence — `parse_regex_default_ast_dump` and `parse_grammar_profile_ast_dump_named` both funnel into the same `run_grammar_parse_ast_dump`). Ledger now: **only `PGEN-RGX-0073` substantively open** (0078 = its superseded re-file).
- **PGEN-RGX-0073 — compile-time perf (the substantive, still-open perf tracker).** This *is* the compile-time issue: PGEN's `regex` parse dominates RGX's total `Regex::compile` wall-clock budget. **Fresh re-measure 2026-05-19 on adopted pin `db6f8c68` (PGEN 1.1.81 / contract 1.1.83), Apple Silicon, default allocator, 8-pattern corpus:** PGEN parse = **≈63–86%** of `Regex::compile` (phase-split), p50 **24–188µs/pattern** (5000 samples), **geomean ≈214× vs PCRE2-10.47-no-JIT compile / ≈32× vs PCRE2+JIT** (same-session C baseline). Trend: raw PGEN parse p50 **~2–3.8× faster** than the PGEN-1.1.40-era baseline; geomean-vs-no-JIT moved ~360× (original filing) → ~214× (now). The "~80×" in older notes was PGEN's own-methodology (mimalloc/internal harness) figure — **not reproduced** by RGX's standard default-allocator measurement; the RGX-side ground truth is the ~214× above (Book → Performance → "Compile-time performance"; artifacts in `pgen-issues/artifacts/PGEN-RGX-0078/measurements/`). Still short of the ROADMAP `<5× of PCRE2 compile` target — closing the residual gap is sustained PGEN-side parser work. **No RGX-side fix and (per user directive 2026-05-19) no new PGEN bug report filed** — the metrics are captured in the docs, the tracker stays open as-is.
...
### C11. rgx-capi header-drift CI gate (STABILITY.md §7) — ✅ DONE 2026-05-18
...
- **Status**: ✅ **shipped 2026-05-18**. `scripts/check-capi-abi.sh` implements both checks: (1) `cargo build -p rgx-capi` then `git diff --quiet -- include/rgx.h` (byte-identical regeneration); (2) "ABI-meaningful" header content (header minus pinned banner + cbindgen tool-stamp) compared vs the `origin/main`/`main` merge-base — if it differs, the `[workspace.package] version` must also differ. Wired into `scripts/run-local-ci.sh` (which **is** the CI gate — the "Workspace checks" job runs it), so no separate ci.yml step needed. Registered in `check-ci-paths.sh` (required-paths + absolute-path audit). STABILITY.md §7/§8 updated (implemented, not reviewer-discipline); book c-abi.md documents the gate. Verified: OK path green; negative test (cbindgen.toml tweak → regeneration drift) → rc=1 with DRIFT message; meaningful()/version awks unit-checked.
- **Effort**: `small`. **Advances**: publish-readiness #1 (its declared "Next concrete step").

### C12. Verify all book code examples (ratcheted campaign) — ✅ COMPLETE 2026-05-18
- **What**: every ```` ```rust ```` block in `book/src/**` must compile (and run, unless `no_run`) so copy-paste works. Census 2026-05-18: 293/297 were `rust,ignore`; the 4 compiled were broken. **✅ DONE**: book-wide `rust,ignore` count is now **0**; 217 `rust` (run) / 60 `no_run` / 1 `should_panic` / 65 `text`; every runnable block is in one of the 34 wired chapters; `--doc` 278/0. Scorecard: ~17 broken examples fixed + 1 PCRE2 engine bug (`\N`/`/s`) + 1 contract clarification (branch-ID bare-top-level-only). Mechanism permanent (gate + ratchet + book.yml deploy-gate).
...
- **Status**: ✅ Foundation + HTTP Router (baseline=1). ✅ **Increment 1 (2026-05-18)**: introduction + why-rgx + getting-started/* (41 blocks, 7 chapters) → **baseline=8**; `cargo test -p rgx-core --doc` 68/0; found+fixed **6 real copy-paste-breaking drifts in published why-rgx.md** (SteerAction→SteerResult, CodeBlockValue::Number→Numeric, with_event_observer→on_event, ctx.text()→current_match(), tail_file `?`, TailOptions path). 4 broken audit blocks honestly re-fenced (foundation). ✅ **Increment 2 (2026-05-18)**: core-api/* (90 blocks, 8 chapters) → **baseline=16**; `cargo test -p rgx-core --doc` 158/0; found+fixed **6 more broken published examples** (3× `unwrap_err()`-on-non-Debug, a `?`/should_panic block, the self-contradictory `is_match_at` example, a `bytes-regex` off-by-one — engine correct in both behavioral cases; all doc bugs). ✅ **Increment 3 (2026-05-18)**: host-integration/* (50 blocks, 6 chapters → `rust,no_run`; c-abi is C, out of scope) → **baseline=22**; `cargo test -p rgx-core --doc` 207/0; 5 incomplete-fragment fixes (no API drift — host-integration API stable). ✅ **Increment 4 (2026-05-18)**: advanced/* (67 blocks, 5 chapters → 63 `rust` run + 2 `rust,no_run` type/trait reproductions + 1 `text` pseudo-fragment) → **baseline=27**; `cargo test -p rgx-core --doc -- book_doctests::Adv` 66/0. **Caught a real PCRE2-conformance engine bug**: `\N` aliased the dotall-sensitive `Regex::Dot` atom at all 4 lowering sites → `(?s)\N` wrongly matched the newline. Root-cause family fix `non_newline_ast()` (RGX bug, not PGEN; no workaround); `(*NUL)` sibling fixed by the same change; regression test added; gate run with `RGX_RUN_CONFORMANCE=1`, ratchet non-regressed. Book prose was correct — engine fixed, book unchanged beyond fencing. ✅ **Increment 5 (2026-05-18)**: remaining real-world/* (21 blocks, 4 chapters → 19 `rust` run + 2 `no_run` IO) → **baseline=31**; `cargo test -p rgx-core --doc -- book_doctests::Rw` 21/0. 2 doc bugs (engine correct per documented contract): tokenizer.md wrapped its classifier alternation in `(?:…)` → `matched_branch_number` all `None` (branch tracking is bare-top-level-only — proven by the new permanent regression `matched_branch_number_requires_bare_top_level_alternation`; stripped the wrapper from all 4 patterns + prose); waf-engine.md `WafRule` sketch had `#[derive(Debug,Clone)]` on a `Regex` field (removed). **Family found** → task #45: host-integration/{data-exchange,match-steering}.md (committed inc3, `no_run`) carry the same `(?:…)` branch-ID overclaim; needs a code-block↔branch-number probe (naive strip breaks `(?{native:…})` precedence) — tracked separately, not forced into this commit. ✅ **Increment 6 (2026-05-18, FINAL)**: appendices/* + internals/* (25 blocks, 10 chapters) → **baseline=34**; `--doc -- book_doctests` 278/0. User-API chapters (execution-modes/pcre2-compatibility/sandboxing) → runnable+wired; the 7 engine-internal design-pseudocode chapters (compilation-pipeline, the-vm, jit-compiler, nfa-dfa-engine, pgen-integration, pcre2-conformance-audit, context-reference) → honest `text` (private types / `…` placeholders / no-body sigs — non-compilable by construction), not wired. No doc bugs in the 3 new chapters. **Campaign #20 COMPLETE.**
- **Remaining**: none. ✅ **#45 branch-ID family (2026-05-18)**: host-integration data-exchange/match-steering rewritten to bare top-level alternation with per-arm code block (engine correct per documented contract; same adjudication as inc-5 tokenizer); prose corrected; pinned by the permanent regression `matched_branch_number_in_code_block_requires_bare_top_level_alternation`. The campaign mechanism is permanent and self-enforcing (book_doctests.rs on the mandatory `cargo test -p rgx-core` gate + `check-book-examples.sh` count-only-grows ratchet + `book.yml` deploy-gate): any new chapter or `rust` block must be verified to land.
