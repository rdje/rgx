...
- **Excluded files** (see harness comments for details):
  - `testinput15` — match-limiting stress file with catastrophic-backtracking patterns (`(a+)*zz`). Some cases don't honor the harness's `max_steps=1M` cap and hang indefinitely. **Root cause identified and fixed 2026-07-21 (`VM-LIMITS-SUBEXEC.1`)**: sub-executions ran off-budget, so the cap was only ever a top-level-loop cap. The follow-up audit is discharged (see A2). Whether this file can now be *re-included* in the corpus is a separate, ratchet-moving question — owned by the **`CONFORMANCE-TESTINPUT15`** task tree, not assumed here.
