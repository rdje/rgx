...
|------|--------|-----|--------|
| **C2 NFA/DFA hybrid** | `major` | Algorithmic class change. "Can't hang" guarantee for the common no-backtracking subset. 10x-100x typical speedup on regular patterns. | **Steps 1-4b shipped** — classifier (1), byte-class partitioning (2), forward + reverse NFA + `CompiledC2Program` (3a+3b), Pike-VM with span tracking + differential gate (4a), Pike-VM with **capture group tracking** + extended differential gate (4b). 12 differential corpus suites passing on `is_match` + spans **+ capture group positions**. All standalone, no engine wiring yet. Step 0 design doc: `docs/C2_NFA_DFA_DESIGN.md`. Next: step 4c (engine dispatch wiring — once landed, the existing 633+ test suite becomes a deeper differential test). 8-step phased plan, multi-week timeline. |
