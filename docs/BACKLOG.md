...
| **C1 JIT compilation** | `major` | Constant-factor multiplier (~5-10x) on whichever engine runs. Sequenced after C2 so wins compound. | **Step 0 COMPLETE 2026-04-11** — comprehensive design proposal landed at `docs/C1_JIT_COMPILATION_DESIGN.md`: Cranelift-based code generator (chosen over dynasm-rs / hand-written / LLVM), JIT'd backtracking VM (NOT the C2 engines on first pass), 4-tier dispatch chain (DFA → JIT → Pike-VM → interpreter), runtime helper layer with stable C ABI, cross-platform validation matrix (P0/P1/P2 priorities for x86_64/aarch64 across Linux/macOS/Windows), 9-step phased plan, 12 open architectural questions with leans, 12 risks with mitigations. Next: sign-off, then step 1 (JIT host plumbing — standalone `c1/` module with `cranelift_jit::JITModule` wrapper, runtime helper skeleton, Cargo `jit` feature flag, smoke test). |
