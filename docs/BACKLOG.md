...
|------|--------|-----|--------|
| **C2 NFA/DFA hybrid** | `major` | Algorithmic class change. "Can't hang" guarantee for the common no-backtracking subset. 10x-100x typical speedup on regular patterns. | **Steps 1-6 COMPLETE** — classifier (1), byte-class partitioning (2), forward + reverse NFA + `CompiledC2Program` (3), sparse-set Pike-VM with engine dispatch (4), lazy forward DFA cache + DFA dispatch for `is_match` (5), DFA dispatch for `find_first`/`find_all` via per-position scan + `pike_captures_at` capture recovery (6). The lazy DFA is now wired into all three primitive Regex API methods. The differential gate is active across the entire 880-test suite for the DFA path. Step 6 took a simpler per-position approach instead of the design-doc-proposed unanchored+reverse-DFA pipeline (which has subtle greedy-semantics issues); the reverse DFA is deferred as a performance optimization. Next: step 7 (literal prefix integration with C2 dispatch). 8-step phased plan, multi-week timeline. |
