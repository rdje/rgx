...
- **Status as of 2026-05-05, head `b170490`**: ratchet locked at **12,716 pass / 94 fail / 0 panic / 0 skip (~99.3%)** against the full `testinput1..29` corpus, post the PGEN 1.1.40 → 1.1.75 bump (commit `42d1809`) plus typed-shape walker migration plus post-bump silent-shape sweep. The residual 94 failures are catalogued per-case in the Book chapter **[PCRE2 Conformance Residual](../book/src/internals/pcre2-conformance-residual.md)**. Buckets: ~30 FN / ~26 SM / 5 FP / 4 too-permissive / 3 substitute-other. Closed since the previous summary: Cluster 2F (quoted-run-as-range-start), Cluster 1G entries for Pattern_White_Space ignorable / `\p{<script>}` Script_Extensions default / U+180E in UCP `\s` / lookaround u8→u16 length / per-subject `\=g` substitute / typed-shape silent gaps surfaced post-bump (boolean `initial_close`, typed `class_quoted_literal`, sub-array body flatten across `quoted_literal` / `class_quoted_literal` / `code_block` / `extract_quoted_class_chars`). The chapter is still the authoritative next-session starting point — most remaining cases are architectural (Cluster 1A recursive captures, Cluster 1B returned-capture subroutine semantics) or structural engine work (`(*NUL)` newline-mode / `/s` interaction, Cluster 2C `\K` propagation from lookarounds).
...
- **C8.1.2 `atomic_depth` field replacing `!ctx.call_stack.is_empty()` predicate** — add `atomic_depth: u32` to `ExecContext`, increment/decrement at `OpCode::AtomicStart`/`AtomicEnd`, flip the `OpCode::Commit` branch test. **Effort**: `trivial` (0.5 day). **Unlocks**: a hidden-but-known correctness gap (`(*COMMIT)` inside a subroutine that isn't itself in an atomic group).
- **C8.1.3 `alt_boundaries` manual-truncation audit** — converge the 8 manual `alt_boundaries` truncation sites with `try_backtrack`'s post-pop cleanup either by routing through a shared helper or by documenting why each manual site is correct. **Effort**: `small` (1 day). **Unlocks**: invariant safety against future opcode additions.
...
- **C8.2.1 Per-verb effects refactor (replaces per-pair patches and scales to N verbs)** —
  - **Phase 1 ✅ shipped 2026-05-06**: Centralized `verb_apply_*` associated functions for all 6 verbs ((*COMMIT), (*PRUNE), (*SKIP), (*SKIP:name), (*THEN), (*MARK:name)) at `rgx-core/src/vm.rs:2200-`; all 3 dispatch sites (top-level, continuation, subexpr) call the same functions. Last-verb-wins precedence encoded inside the apply functions: `verb_apply_skip` clears `committed`, `verb_apply_prune` clears both `skip_position` and `committed`, `verb_apply_then` (redirect path) clears `skip_position` and `committed`, `verb_apply_commit` (non-atomic) clears `skip_position`. `ThenOutcome` enum signals redirect-vs-degrade for the subexpr cross-context cleanup. Engine fixes #24 / #36 / 2026-05-05 SKIP-overrides-COMMIT collapsed into rows of the apply table. Conformance ratchet unchanged at 12,719/91 — semantic refactor with two case shifts between FN/SM categories.
  - **Phase 2 (pending)**: Defer the stack-clear effect of (*COMMIT)/(*SKIP) so that a subsequent (*THEN) in the same branch can still find the alt-fallback frame. The current apply functions clear the active backtrack stack immediately; (*THEN) cannot redirect to alt2 because the alt-frame was cleared by (*COMMIT)/(*SKIP) before (*THEN) fired. Phase 2 reads the verb-state at failure time (in `try_backtrack` or the scanning-loop tail) instead of mutating the stack at apply time. Closes residual Cluster 1D testinput1:5457 (`aaaaa(*COMMIT)(*THEN)b|a+c` → "aaaaaac") and the SKIP+THEN sibling testinput1:5447 by construction. **Effort**: `small-medium` (1-3 days). **Unlocks**: ~2-4 Cluster 1D cases plus retires the held `commit_saved_alt` proposal as a special case.
