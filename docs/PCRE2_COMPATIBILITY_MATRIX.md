...
- Current shipped Perl extended character class subset: bracket/property terms, bare ASCII POSIX class terms (such as `[:alpha:]`, `[:graph:]`, `[:digit:]`, `[:space:]`, `[:word:]`, and the other current ASCII forms), bare shorthand terms (`\d`, `\D`, `\w`, `\W`, `\s`, `\S`, `\h`, `\H`, `\v`, `\V`), bare escaped literal/codepoint terms such as `\n`, `\t`, `\r`, `\x{41}`, `\x41`, and `\-`, unary complement, grouped subexpressions, and same-level left-associative set algebra with `&` binding tighter than `|` / `+` / `-` / `^`
