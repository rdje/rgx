...
- The current PGEN-backed integration now depends on the committed `subs/pgen` submodule:
  - the pinned fix commit is `54ed190437371fdcc8e77751407f5b3d51efbd52` (PGEN 1.1.8, includes braced octal, numeric angle subroutine, and tagged code-block fixes)
...
- Bare top-level ordinary terms inside Perl extended character classes, such as `(?[a-z])` and `(?[\dA-F])`, remain intentionally unsupported because a local PCRE2 parity probe compile-rejected those forms; the shipped ordinary-term slice still uses nested bracket forms such as `(?[[a-z]])` and `(?[[\dA-F]])`.
...
- Quick benchmark trend capture is now part of the default validation loop through `scripts/capture-benchmark-trends.sh`; each run now preserves shared plus mode-scoped latest snapshots, writes a cross-mode `overview.*` that also surfaces the newest shared quick/full label pair, writes label-paired quick/full summaries to `profile-pairs.*`, writes rolling label-pair history to `profile-history.*` with latest-pair improvement/regression callouts, writes rolling history summaries (`history-quick.*` / `history-full.*`), archives timestamped local history under `history/quick` or `history/full`, records an optional capture label (`--label` / `RGX_BENCHMARK_TREND_LABEL`) that the wrapper defaults from the current git revision, and reports deltas only against same-mode baselines unless an explicit archived baseline is requested (`RGX_BENCHMARK_COMPARE_AGAINST=<unix>` or `RGX_BENCHMARK_COMPARE_AGAINST=label:<text>`); the internal path/write/log plumbing for those artifacts is now centralized in `trend_capture.rs` so future report additions do not have to duplicate file-layout logic at each output site, while broader release-profile tracking beyond those same-label quick/full longitudinal summaries remains a separate follow-up

- **Compile-pipeline stack safety (2026-05-18).** The whole compile path is recursive once per pattern-nesting level: PGEN's recursive-descent parse + its recursive `serde_json` AST-dump serialization (both inside `pgen::embedding_api::parse_regex_default_ast_dump`), then RGX's typed-AST walker (`parsing.rs::convert_typed_pattern` choke point) and ~14 recursive `compiler.rs` passes + codegen. Only the JSON *deserialize* step was stack-protected (`serde_stacker`, added 2026-04-07 `c313568`) — a **half-applied family fix**: every other recursive descent was unprotected, so a deeply nested pattern overflowed the thread stack and aborted the *host process* (SIGABRT), nondeterministically (serde_stacker heuristic-growth signature: crash depth non-monotonic). Closed RGX-side 2026-05-18: `rgx-core/src/recursion.rs` enforces `MAX_NESTING_DEPTH = 1000` (4× PCRE2's 250 / `regex`'s `nest_limit`) via an O(n) non-recursive pre-PGEN string scan returning a clean `CompileError`, and runs the within-limit parse/compile under `stacker` (`compile_on_deep_stack`) — exact parity with the existing `serde_stacker` JSON treatment, NOT a doctrine-forbidden AST-absorption workaround (PGEN output consumed unchanged; no builtin-parser fallback). PGEN owns the real fix (its own parser recursion guard) — `pgen-issues/PGEN-RGX-0085.yaml`. **Lesson**: when adding a recursion guard, fix the *whole* recursive family in one go (per `feedback_family_fix_doctrine`); a point-fix on one recursive site silently rots as the rest of the pipeline grows.
- **Validation-gap lesson (2026-05-18).** The above shipped broken for ~6 weeks because the COMMIT.md mandatory gate `cargo test -p rgx-core` (which runs lib **+ integration** suites incl. `stress_tests`) was being satisfied with targeted/`--lib`-only runs, and hosted CI cannot build at all (workflow pins toolchain 1.88.0 while `Cargo.toml` MSRV is 1.95 — cargo hard-errors). `cargo test -p rgx-core` means the **full** suite on the **default thread stack**; a filtered subset is not the gate. **Closed 2026-05-18**: CI toolchain pinned to MSRV 1.95.0 (`1c4c670`); a gate-receipt guard shipped — `scripts/run-local-ci.sh` writes a tree-stamped receipt only on a real green (every step under `set -euo pipefail`, no exit-masking), and a tracked `scripts/git-hooks/pre-commit` (one-time `./scripts/setup-hooks.sh`) blocks committing a gate-affecting tree without a fresh matching receipt. Building it surfaced a fourth root cause — `check-ci-paths.sh`'s `[A-Za-z]:\\` Windows heuristic false-matched `:\n`/`:\x` in Rust format strings, so the canonical gate runner had itself been red on benign source (fixed → require escaped `[A-Za-z]:\\\\`). The mechanism caught the masking trap in live fire on first run (a "exit 0" notification while the gate had failed and no receipt was written). **Standing rule**: never conclude "pass" from filtered/piped output — assert the real status (`${PIPESTATUS[0]}` / unpiped / let `run-local-ci.sh` do it). See COMMIT.md step 2 and the book Testing Philosophy "Enforcing the gate" section.

- **Match-time limits must cover every dispatch loop, not "the" dispatch loop (2026-07-21, `VM-LIMITS-SUBEXEC.1`).** `set_max_steps` / `set_max_backtrack_frames` / `set_max_trail_entries` shipped as a whole-attempt DoS contract but were implemented only in `execute_at`. The VM actually has **three** execution loops — `execute_at`, `execute_subexpr_inner_full` (assertion bodies, lookbehind candidate starts, quantifier bodies, subroutine/recursion bodies, conditional/atomic probes), and `execute_at_continuation` (async resume) — and the other two neither incremented nor consulted the counter; `execute_subexpr_inner_full`'s **local** `backtrack_stack` was likewise never checked. Compounding it, `clone_exec_context` copied `step_count` *into* a speculative sub-context and nothing copied it *back*, so each clone restarted the budget: a variable-length lookbehind clones once per candidate start, giving `starts.len()` × an already-unbounded body. Net effect: `/(?<=(\d{1,256}))X/` burned 20+ minutes on an 8-byte subject with a 1M-step cap set. Fixed by making the invariant explicit — **one attempt, one budget**: a shared `step_budget_exhausted` predicate used by all three loops, and `absorb_sub_budget` folding a sub-context's spend back at all three `clone_exec_context` sites. **A budget that was never honest was also never calibrated.** The first full-gate run after the fix went red: two legitimate conformance cases (`testinput1:4545`, the recursive palindrome) started failing because the harness's 1M step cap had been chosen while sub-execution work was invisible. Measured properly, that case needs ~33M steps. The cap was recalibrated (1M → 64M, frames/depth verified non-binding by bisecting each limit alone) rather than the ratchet rebaselined — the cases match in PCRE2, so a baseline bump would have booked an accuracy regression as progress. Expect this whenever instrumentation becomes honest: every threshold tuned against the old, under-counting instrument is now wrong and must be re-derived from measurement, not adjusted until the red goes away. **Lessons**: (1) a resource limit is a property of an *invariant*, not of a function — enumerate every path that can execute opcodes (sweep `OpCode::try_from(code[ip])`, discounting static walkers) and every path that *copies execution state*, because a copied budget that is never returned is the same bug as no budget at all; (2) an exclusion comment is evidence — the conformance harness had already documented "some cases don't honor the cap" as the reason `testinput15` is excluded, which was this bug reported and then normalized rather than root-caused (now `CONFORMANCE-TESTINPUT15`); (3) write DoS regression tests with a worker-thread + `recv_timeout` guard so a regression *fails* with a diagnostic instead of hanging CI, which is how this class of bug hides.
...
3. Deepen the new mode-scoped benchmark-trend capture beyond the shipped same-label quick/full pairing plus rolling paired-label history into a fuller release-profile / longitudinal comparison story and prioritize measurable wins
