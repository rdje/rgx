...
- Quick benchmark trend capture is now part of the default validation loop through `scripts/capture-benchmark-trends.sh`; each run now preserves shared plus mode-scoped latest snapshots, writes a cross-mode `overview.*`, writes label-paired quick/full summaries to `profile-pairs.*`, writes rolling label-pair history to `profile-history.*` with latest-pair improvement/regression callouts, writes rolling history summaries (`history-quick.*` / `history-full.*`), archives timestamped local history under `history/quick` or `history/full`, records an optional capture label (`--label` / `RGX_BENCHMARK_TREND_LABEL`) that the wrapper defaults from the current git revision, and reports deltas only against same-mode baselines unless an explicit archived baseline is requested (`RGX_BENCHMARK_COMPARE_AGAINST=<unix>` or `RGX_BENCHMARK_COMPARE_AGAINST=label:<text>`), while broader release-profile tracking beyond those same-label quick/full longitudinal summaries remains a separate follow-up
