...
- The current live PGEN regex caveats are narrower than the original complaint set:
  ...
  - named recursion-condition syntax `(?(R&name)...)` is not parser-transportable yet on the pinned `1.1.1` backend and is now tracked locally as `pgen-issues/PGEN-RGX-0005.yaml`,
...
- The next named recursion-condition slice `(?(R&name)...)` is currently blocked by the default parser transport layer rather than by RGX runtime design: the pinned PGEN backend rejects the minimal sample `(?(R&word)a|b)` at byte 0, and the local upstream-style bundle is tracked as `pgen-issues/PGEN-RGX-0005.yaml`
