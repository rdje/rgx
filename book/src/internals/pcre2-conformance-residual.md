...
| testinput2:6244 | `\A\s*(a\|(?:[^\`]{28500}){4})/I` | `a` | JIT step-limit interaction (verified 2026-05-05). Without `set_max_steps`, RGX matches `a` correctly via the simple alt. With ANY non-zero limit (even `max_steps=10` — well below the 1M harness default), the JIT path returns no-match. The harness sets `max_steps=1_000_000`, so this looks like a no-match in the conformance suite. Root cause not yet pinned: setting a limit may force a different code path that doesn't try the simple alternative first, or the JIT compile of the giant `[^\`]{28500}` body emits per-op step-counter checks that fire against the giant bytecode regardless of execution path. **What to change**: investigate `try_jit_find_first` divergence on limit-vs-no-limit for this pattern shape; the matching `a` alt should reach SUCCESS in <10 ops. |
