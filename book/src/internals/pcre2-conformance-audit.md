...
The conformance ratchet is currently locked at **12,805 pass / 5 fail / 0 panic / 0 skip** against the full PCRE2 10.47 `testinput1..29` corpus — **~99.96%**. (It was 12,806 / 4; **deliberately rebaselined −1 on 2026-05-18** when the PGEN combined pin `65b845f0` (rel 1.1.77) was adopted: +1 from PGEN-RGX-0084's `\10` case closing, −2 from the tracked PGEN-side PGEN-RGX-0087 `\8N`/`\9N` regression — the conformance harness's sanctioned "intentional + justified" path, restoring toward 12,807/3 on PGEN's 0087 fix.) That baseline is the result of roughly 270 commits over 28 days, of which about 130 directly touched conformance behaviour. Pass-count progression visible from `CHANGES.md`: 1,061 (initial harness, testinput1 only) → 3,613 (full-corpus expansion) → 12,562 (~98% after engine fix #9) → 12,720 (2026-05-06, per-verb effects Phase 2) → 12,737 (2026-05-07) → 12,800 (2026-05-08, subroutine retry-shorter for `StarGreedy(Call)`) → 12,801 (2026-05-11 AM, retry-different on `Call`-followed-by-backref) → 12,805 (2026-05-11, `SubroutineRetryMode::Shorter|Different` split closes deep-palindrome family) → **12,806** (2026-05-11, scope `(*THEN)` to its subroutine call closes testinput2:3350). The work decomposes into three mostly disjoint tracks running in parallel:
...
- **PCRE2 source of truth**: `subs/pcre2` submodule pinned to 10.47 (commit `f454e231`); `subs/pgen` submodule pinned to PGEN 1.1.77 (commit `65b845f0`, contract 1.1.79).
