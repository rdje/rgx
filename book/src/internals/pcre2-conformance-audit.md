...
The conformance ratchet is currently locked at **12,801 pass / 9 fail / 0 panic / 0 skip** against the full PCRE2 10.47 `testinput1..29` corpus — **~99.93%**. That number is the result of roughly 270 commits over 28 days, of which about 130 directly touched conformance behaviour. Pass-count progression visible from `CHANGES.md`: 1,061 (initial harness, testinput1 only) → 3,613 (full-corpus expansion) → 12,562 (~98% after engine fix #9) → 12,720 (2026-05-06, per-verb effects Phase 2) → 12,737 (2026-05-07) → 12,800 (2026-05-08, subroutine retry-shorter for `StarGreedy(Call)`) → **12,801** (2026-05-11, subroutine retry-different on `Call`-followed-by-backref closes the `^((.)(?1)\2|.?)$` palindrome family). The work decomposes into three mostly disjoint tracks running in parallel:
