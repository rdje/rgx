...
The conformance ratchet is currently locked at **12,806 pass / 4 fail / 0 panic / 0 skip** against the full PCRE2 10.47 `testinput1..29` corpus — **~99.97%**. (It was rebaselined to the interim 12,805/5 on the 2026-05-18 `65b845f0` adoption while PGEN-RGX-0087 was open; on 2026-05-19 PGEN rel 1.1.80 (`5fd20609`) closed 0087 and the ratchet was rebaselined back up to 12,806/4. Later that day PGEN rel 1.1.81 (`db6f8c68`) closed **PGEN-RGX-0088** via a scoped revert — PGEN now emits octal `>0o377` mode-agnostically as a code point. The ratchet was **held, not rebaselined**: closing 0088's `testinput10:218` re-exposes `testinput9:287` (`/(?i:A{1,}\6666666666)/` under `testinput9` = "8-bit library, `#forbid_utf`"), a 1↔1 swap that stays at 12,806/4. `testinput9:287` is a **permanent by-design residual** — RGX is a Unicode/code-point engine with no 8-bit non-UTF mode, so it accepts `\666`=U+01B6 exactly as PCRE2-`,utf` would, while PCRE2's 8-bit library raises error 151; both spec-correct for their model, not a 0088 reopen, not a PGEN bug, not an RGX defect — see [PCRE2 Conformance Residuals → Bucket 6 / Cluster 6A](pcre2-conformance-residual.md). RGX carries zero known PGEN-tracked regressions.) That baseline is the result of roughly 270 commits over 28 days, of which about 130 directly touched conformance behaviour. Pass-count progression visible from `CHANGES.md`: 1,061 (initial harness, testinput1 only) → 3,613 (full-corpus expansion) → 12,562 (~98% after engine fix #9) → 12,720 (2026-05-06, per-verb effects Phase 2) → 12,737 (2026-05-07) → 12,800 (2026-05-08, subroutine retry-shorter for `StarGreedy(Call)`) → 12,801 (2026-05-11 AM, retry-different on `Call`-followed-by-backref) → 12,805 (2026-05-11, `SubroutineRetryMode::Shorter|Different` split closes deep-palindrome family) → **12,806** (2026-05-11, scope `(*THEN)` to its subroutine call closes testinput2:3350). The work decomposes into three mostly disjoint tracks running in parallel:
...
| 2026-05-18 | PGEN bump 1.1.75 → 1.1.77 (`65b845f0`, combined 0084/0085/0086) | net −1 (rebaseline 12,806/4→12,805/5) | schema `1` unchanged (no walker change). 0084/0085/0086 closed; +1 (0084 `\10`/testinput1:3910) −2 (NEW PGEN-RGX-0087 `\8N`/`\9N` regression, testinput2:4671/:4674) — deliberate justified rebaseline |
| 2026-05-19 | PGEN bump 1.1.77 → 1.1.80 (`5fd20609`; 0087 FIX2.1+.2+.3) | net +1 (rebaseline 12,805/5→**12,806/4**) | schema `1` unchanged. PGEN-RGX-0087 CLOSED (4671/4674 + class-context all PCRE2-faithful; rel-1.1.78 interim over-reject caught+reported). +2 (0087) −1 (NEW PGEN-RGX-0088: FIX2.3 unconditional octal `>\377` reject over-rejects UTF-valid octal, testinput10:218). Unit test rewritten PCRE2-faithful + un-`#[ignore]`d |
| 2026-05-19 | PGEN bump 1.1.80 → 1.1.81 (`db6f8c68`; 0088 scoped revert) | net 0 (**no rebaseline**, 1↔1 swap, stays 12,806/4) | schema `1` unchanged. PGEN-RGX-0088 CLOSED — rel-1.1.81 reverts FIX2.3's unconditional octal `>\377` reject; PGEN now emits octal `>0o377` mode-agnostically as a code point (range/mode is an engine-time decision — `feedback_ast_pipeline_parser_agnostic`). `testinput10:218` CLOSES, `testinput9:287` (`/(?i:A{1,}\6666666666)/`, `testinput9` = 8-bit-library `#forbid_utf`) RE-EXPOSES — now a **permanent by-design residual** (RGX is Unicode-only; not a PGEN bug, not an RGX defect — see Bucket 6 / Cluster 6A). New regression test `octal_escape_over_0o377_is_a_codepoint_not_a_parse_reject` |
...
Of the 88 `PGEN-RGX-NNNN` reports filed (`pgen-issues/PGEN-RGX-0001.yaml` through `PGEN-RGX-0088.yaml`), 87 are closed and only `PGEN-RGX-0073` — PGEN parse compile-time perf — remains substantively open as a non-conformance issue (`PGEN-RGX-0078` is its superseded PCRE2-relative re-file). The later clusters all closed on the adopted pin `db6f8c68` (PGEN 1.1.81): the `\NN…` octal-vs-backref family 0084/0085/0086/0087/0088 and the early typed-shape family 0079/0080/0081/0082. `PGEN-RGX-0014` is a renumbering (not present on disk; the report-numbering campaign began with `PGEN-RGX-0015`), and several mid-campaign reports were retroactively flipped to closed during the 2026-04-24 user directive that declared the report-tracking surface complete. None of these reports introduced an RGX-side workaround; every one closed via a submodule bump and an adapter walker update.
...
- **PGEN-issue tracker**: `pgen-issues/PGEN-RGX-NNNN.yaml`. 88 filed, 87 closed; only `PGEN-RGX-0073` (compile-time perf) remains open and is non-conformance (`PGEN-RGX-0078` was its superseded PCRE2-relative re-file). The `\NN…` octal-vs-backref cluster 0084/0085/0086/0087/0088 is fully resolved. The early typed-shape cluster 0079 (`\o{…}` non-octal-digit reject), 0080 (counted-quantifier inner whitespace), 0081 (`\g` subroutine-vs-backreference typed `kind`), 0082 (native `code_block` content) was verified-closed on `db6f8c68` (2026-05-19) — fresh-binary probes cross-checked by the green conformance ratchet.
- **PCRE2 source of truth**: `subs/pcre2` submodule pinned to 10.47 (commit `f454e231`); `subs/pgen` submodule pinned to PGEN 1.1.81 (commit `db6f8c68`, contract 1.1.83).
