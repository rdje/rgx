...
The conformance ratchet is currently locked at **12,719 pass / 91 fail / 0 panic / 0 skip** against the full PCRE2 10.47 `testinput1..29` corpus — **~99.3%**. That number is the result of roughly 240 commits over 23 days, of which about 110 directly touched conformance behaviour. Pass-count progression visible from `CHANGES.md`: 1,061 (initial harness, testinput1 only) → 3,613 (full-corpus expansion) → 12,562 (~98% after engine fix #9) → 12,719 (current). The work decomposes into three mostly disjoint tracks running in parallel:
