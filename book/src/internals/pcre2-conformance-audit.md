...
The conformance ratchet is currently locked at **12,805 pass / 5 fail / 0 panic / 0 skip** against the full PCRE2 10.47 `testinput1..29` corpus — **~99.96%**. That number is the result of roughly 270 commits over 28 days, of which about 130 directly touched conformance behaviour. Pass-count progression visible from `CHANGES.md`: 1,061 (initial harness, testinput1 only) → 3,613 (full-corpus expansion) → 12,562 (~98% after engine fix #9) → 12,720 (2026-05-06, per-verb effects Phase 2) → 12,737 (2026-05-07) → 12,800 (2026-05-08, subroutine retry-shorter for `StarGreedy(Call)`) → 12,801 (2026-05-11 AM, retry-different on `Call`-followed-by-backref) → **12,805** (2026-05-11, `SubroutineRetryMode::Shorter|Different` split with `attempts_left` budget closes the `^(.|(.)(?1)\2)$` deep-palindrome family). The work decomposes into three mostly disjoint tracks running in parallel:
