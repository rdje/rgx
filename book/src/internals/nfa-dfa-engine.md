...
- **Reverse-DFA pipeline** — the reverse NFAs are built and stored on `CompiledC2Program`, but the dispatch path still uses per-position anchored scans for `find_first` / `find_all`. The reverse-DFA approach is faster for sparse-match patterns on long inputs but has subtle "earliest end vs longest end" semantics that need careful handling. It's a future optimization.
