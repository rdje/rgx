...
- The **reverse anchored** NFA backs the reverse half of the reverse-DFA pipeline via `LazyDfa::find_match_start_at_reverse`. It is now wired for `find_first`: the pipeline runs as **forward-unanchored DFA (stop at first accept) → reverse-anchored DFA (leftmost start) → forward-anchored DFA (greedy end) → Pike-VM captures**. The third pass is what lets greedy tails like `a+` on `"baaab"` extend the match beyond the first-accept position (end=2) out to the greedy end (end=4); without it, the pipeline would truncate greedy matches. Dispatch for `find_first` only prefers the pipeline when no prefix hint exists (`c2_prefix_byte == None` and `PrefixFilter::None`) — otherwise the per-position anchored scan with `memchr` / byte-class skip acceleration wins on prefix-rich patterns because it jumps directly to candidate positions. `find_all` still uses the per-position scan. The **reverse unanchored** NFA is reserved for follow-up work.
...
- **Reverse-DFA pipeline** — both halves shipped. `Engine` builds a **forward-unanchored** and a **reverse-anchored** `LazyDfa` alongside the existing forward-anchored DFA, and the pipeline is wired for `find_first`: one O(n) forward-unanchored walk stops at the first accept (leftmost-first end hint), the reverse-anchored DFA walks backward to locate the leftmost start, and a final forward-anchored pass from that start captures the greedy extension (e.g. `a+` on `"baaab"` takes end from 2 out to 4). Pike-VM then recovers captures bounded to the known span, and its end offset cross-checks the pipeline's greedy end before the result is returned. The forward-unanchored NFA carries `lazy_prefix_states` / `body_entry` metadata so DFA subset construction re-runs the epsilon closure excluding lazy-prefix states once accept is in the set — the leftmost-first signal at the state-set level. A `LazyDfa::find_first_accept_at` method delivers the stop-at-first-accept contract. Dispatch prefers the pipeline only when no prefix hint is available (`c2_prefix_byte.is_none()` and `PrefixFilter::None`) so prefix-rich patterns keep the faster memchr / byte-class skip scan. `find_all` still runs the per-position scan; promoting it onto the pipeline is the follow-up.
