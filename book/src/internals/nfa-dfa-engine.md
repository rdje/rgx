...
- The **forward unanchored** NFA wraps the pattern with a lazy `(?s:.)*?` prefix and is used by Pike-VM `is_match` so a single pass over the input can answer the boolean question.
- The **reverse anchored** and **reverse unanchored** NFAs are reserved for the future reverse-DFA pipeline (forward DFA finds the match end → reverse DFA finds the match start → bounded Pike-VM recovers captures). The current implementation uses the simpler per-position scan instead, but the reverse NFAs are built and ready.
...
- **Reverse-DFA pipeline** — the reverse NFAs are built and stored on `CompiledC2Program`, and the **foundation** has landed: `Engine` builds a reverse-anchored `LazyDfa` alongside the forward DFA in `Engine::new`, exposed via `should_dispatch_to_reverse_dfa`, with a new `LazyDfa::find_match_start_at_reverse` method that walks backward from a known endpoint to recover the leftmost start. The dispatch path still uses per-position anchored scans for `find_first` / `find_all`; wiring the new pipeline (forward DFA finds end → reverse DFA finds start → bounded Pike-VM recovers captures) into the dispatch chain is the next step. The reverse-DFA approach is faster for sparse-match patterns on long inputs but has subtle "earliest end vs longest end" semantics that need careful handling — they're tied to the leftmost-longest-vs-leftmost-first divergence the C1 step 6 differential gate exposed for negated character classes.
